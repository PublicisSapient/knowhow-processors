...
import com.publicissapient.kpidashboard.jiratest.adapter.atlassianbespoke.parser.CustomSearchResultJsonParser;
import com.publicissapient.kpidashboard.jiratest.util.JiraConstants;

import io.atlassian.util.concurrent.Promise;

public class ProcessorAsynchSearchRestClient extends AbstractAsynchronousRestClient
	...
	private final CustomSearchResultJsonParser searchResultJsonParser =
			new CustomSearchResultJsonParser();
	...
	private final GenericJsonArrayParser<Filter> filtersParser =
			GenericJsonArrayParser.create(new FilterJsonParser());
	 ...
	 * initialize URIs
	 *
	 * @param baseUri Jira Base URI
	 * @param asyncHttpClient Async http Client
	 */
	public ProcessorAsynchSearchRestClient(final URI baseUri, final HttpClient asyncHttpClient) {
		...
		this.favouriteUri =
				UriBuilder.fromUri(baseUri).path(JiraConstants.FILTER_FAVOURITE_PATH).build();
	}

	/**
	 * Searches JQL
	 *
	 * @param jql Serach Jql query
	 */
	@Override
	 ...
	 * @param startAt search page start at
	...
	private Promise<SearchResult> searchJqlImplPost(
		...
		try {
			postEntity
					.put(JiraConstants.JQL_ATTRIBUTE, jql)
			...
			if (fields != null) {
				postEntity.put(
						JiraConstants.FIELDS_ATTRIBUTE, fields); // putOpt doesn't work with collections
			}
	...
	public Promise<Filter> getFilter(long id) {
		return getFilter(
				UriBuilder.fromUri(baseUri)
						.path(String.format(JiraConstants.FILTER_PATH_FORMAT, id))
						.build());
	}
}
