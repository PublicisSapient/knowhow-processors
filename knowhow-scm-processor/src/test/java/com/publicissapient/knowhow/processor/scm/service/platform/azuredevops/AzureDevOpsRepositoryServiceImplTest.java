...
class AzureDevOpsRepositoryServiceImplTest {
	...
	@InjectMocks private AzureDevOpsRepositoryServiceImpl service;
	...
	void fetchRepositories_Success() throws Exception {
		...
		verify(azureDevOpsClient)
				.fetchRepositories(eq("testToken"), eq("testOrg"), any(), eq(connectionId));
	}
	...
	void fetchRepositories_WithHttpUrl() throws Exception {
		...
		assertNotNull(result);
		verify(azureDevOpsClient)
				.fetchRepositories(eq("testToken"), eq("myOrg"), any(), eq(connectionId));
	}
