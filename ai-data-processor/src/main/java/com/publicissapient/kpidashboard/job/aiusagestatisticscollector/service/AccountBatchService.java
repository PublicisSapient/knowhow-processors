...
public class AccountBatchService {
	...
	public void initializeBatchProcessingParametersForTheNextProcess() {
		allAccounts =
				accountHierarchyRepository.findDistinctByLabel(HIERARCHY_LABEL).stream()
	...
	/**
	 * Return next account to process as a AIUsagePerOrgLevel. Each page contains exactly 1 account
	 * because the endpoint supports only 1 account per request.
	 */
	public AIUsagePerOrgLevel getNextAccount() {
		...
		AIUsagePerOrgLevel page =
				new AIUsagePerOrgLevel(
						"account",
						account.getNodeName(),
						Instant.now(),
						null,
