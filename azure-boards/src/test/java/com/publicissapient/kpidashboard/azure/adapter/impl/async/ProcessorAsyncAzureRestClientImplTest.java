...
public class ProcessorAsyncAzureRestClientImplTest {
	...
	public void init() {
		...
		restClient =
				processorAsyncAzureRestClient =
						new ProcessorAsyncAzureRestClientImpl(
								restOperationsFactory, azureProcessorConfig, mapper);
	...
	public void getUpdatesResponse()
		...
		String authHeader =
				"Basic " + new String(Base64.encodeBase64(userInfo.getBytes(StandardCharsets.US_ASCII)));
		...
		Mockito.when(
						rest.exchange(
								...
								ArgumentMatchers.eq(new HttpEntity<>(headers)),
								ArgumentMatchers.eq(String.class)))
				.thenReturn(response);
		AzureUpdatesModel actualModel =
				processorAsyncAzureRestClient.getUpdatesResponse(azureServer, "92");
	...
	public void getWiqlResponse()
		...
		Mockito.when(
						rest.exchange(
								...
								ArgumentMatchers.eq(String.class)))
				.thenReturn(response);
		AzureWiqlModel actualModel =
				processorAsyncAzureRestClient.getWiqlResponse(
						azureServer, time, projectConfFieldMapping, false);
	...
	private void prepareProjectConfigFieldMapping()
	...
	public void getWorkItemInfo()
		...
		AzureBoardsWIModel actualModel =
				processorAsyncAzureRestClient.getWorkItemInfo(azureServer, ids);
	...
	@Test
	public void getIterationsResponse()
			throws RestClientException,
					URISyntaxException,
					JsonParseException,
					JsonMappingException,
		...
		ResponseEntity<String> response =
				new ResponseEntity<>(createIterationsResponse(), HttpStatus.OK);
		Mockito.when(
						rest.exchange(
								ArgumentMatchers.any(URI.class),
								ArgumentMatchers.eq(HttpMethod.GET),
								...
								ArgumentMatchers.eq(String.class)))
				.thenReturn(response);
		AzureIterationsModel actualModel =
				processorAsyncAzureRestClient.getIterationsResponse(azureServer);
	 ...
	 */
	@Test
	public void getWiqlResponse_prepareQuery()
			throws RestClientException,
					URISyntaxException,
					JsonParseException,
					JsonMappingException,
	...
	public void getWiqlResponse_prepareQuery_isDataExistTrue()
		...
		AzureWiqlModel actualModel =
				processorAsyncAzureRestClient.getWiqlResponse(
						azureServer, time, projectConfFieldMapping, true);
		AzureWiqlModel expectedModel = new AzureWiqlModel();
		expectedModel.setAsOf("2020-07-17T12:01:50.663Z");
		assertEquals(expectedModel.getAsOf(), actualModel.getAsOf(), "Checking for values");
		azureToolConfig.setBoardQuery(
