...
public class RallyRestClient {
	...
	private Connection getConnection(ProjectConfFieldMapping projectConfig) {
		...
		if (projectConfig.getProjectToolConfig() == null) {
			log.error(
					"ProjectToolConfig is null for project: {}",
		...
		if (projectConfig.getProjectToolConfig().getConnectionId() != null) {
			return connectionRepository
					.findById(projectConfig.getProjectToolConfig().getConnectionId())
					.orElse(null);
		}
		return null;
	}
		...
		if (parsedResponse instanceof RallyTypeDefinitionResponse) {
			...
			if (response.getQueryResult() != null && !response.getQueryResult().getErrors().isEmpty()) {
				...
				throw new RuntimeException(
						"Rally API returned errors: " + response.getQueryResult().getErrors()); // NOSONAR
			}
		}
	...
	public <T> ResponseEntity<T> get(
			String url, ProjectConfFieldMapping projectConfig, Class<T> responseType)
			throws JsonProcessingException {
		try {
			HttpHeaders headers = createHeaders(projectConfig);
			if (headers.isEmpty()) {
				log.error(
						"No access token found for connection. ProjectToolConfig may be null or missing connectionId");
				return null;
			}
			...
			if (rawResponse.getBody() != null) {
				...
				log.info(
						"Successfully parsed Rally API response to type: {}", responseType.getSimpleName());
