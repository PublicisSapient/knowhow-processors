...
class GitHubMergeRequestServiceImplTest {
	...
	@Mock private GitHubCommonHelper commonHelper;

	@Mock private GHPullRequest ghPullRequest;

	@Mock private GHRepository ghRepository;

	@Mock private GHCommitPointer head;

	@Mock private GHCommitPointer base;
	...
	void fetchMergeRequests_success() throws Exception {
		...
		GitHubCommonHelper.PullRequestStats stats =
				new GitHubCommonHelper.PullRequestStats(100, 5, 10, 60, 40);
	...
	void fetchMergeRequests_withBranchFilter() throws Exception {
		...
		List<ScmMergeRequests> result =
				service.fetchMergeRequests(toolConfigId, gitUrlInfo, "main", "token", since, until);
	...
	void fetchMergeRequests_branchFilterExcludesPR() throws Exception {
		...
		List<ScmMergeRequests> result =
				service.fetchMergeRequests(toolConfigId, gitUrlInfo, "develop", "token", since, until);
	...
	void fetchMergeRequests_emptyBranchName() throws Exception {
		...
		List<ScmMergeRequests> result =
				service.fetchMergeRequests(toolConfigId, gitUrlInfo, "   ", "token", since, until);
