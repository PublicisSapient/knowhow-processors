...
class AzureDevOpsRateLimitMonitorTest {
	...
	@InjectMocks private AzureDevOpsRateLimitMonitor azureDevOpsRateLimitMonitor;
	...
	void testCheckRateLimit_WithDevAzureUrl_Success() {
		try (MockedConstruction<Connection> mockedConnection =
				mockConstruction(
						Connection.class,
						(mock, context) -> {
			...
			RateLimitStatus status =
					azureDevOpsRateLimitMonitor.checkRateLimit(TEST_TOKEN, TEST_BASE_URL_DEV);
	...
	void testCheckRateLimit_WithVisualStudioUrl_Success() {
		try (MockedConstruction<Connection> mockedConnection =
			...
			RateLimitStatus status =
					azureDevOpsRateLimitMonitor.checkRateLimit(TEST_TOKEN, TEST_BASE_URL_VS);
