...
class DataReferenceUpdaterTest {
	...
	@InjectMocks private DataReferenceUpdater dataReferenceUpdater;
	...
	void testUpdateMergeRequestsWithUserReferences_ValidAuthorAndReviewers_Success() {
		...
		dataReferenceUpdater.updateMergeRequestsWithUserReferences(
				mergeRequests, userMap, repositoryName);
	...
	void testUpdateMergeRequestsWithUserReferences_AuthorNotInMap_CreatesNewUser() {
		...
		when(persistenceService.findOrCreateUser(
						eq(repositoryName), eq("newauthor"), eq("newauthor@test.com"), eq("New Author"), any()))
				.thenReturn(newUser);
		...
		verify(persistenceService, times(1))
				.findOrCreateUser(anyString(), anyString(), anyString(), anyString(), any());
	}

	@Test
	void
			testUpdateMergeRequestsWithUserReferences_PersistenceException_ThrowsDataProcessingException() {
		// Arrange
		ScmMergeRequests mr = new ScmMergeRequests();
		User tempAuthor = new User();
