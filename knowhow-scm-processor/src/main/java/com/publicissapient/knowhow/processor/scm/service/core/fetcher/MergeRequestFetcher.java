...
/**
 * Responsible for fetching merge requests with optimized logic. Follows Single Responsibility
 * Principle.
 */
@Component
@Slf4j
public class MergeRequestFetcher {
	...
	public MergeRequestFetcher(
			MergeRequestServiceLocator mergeRequestService,
	...
	public List<ScmMergeRequests> fetchMergeRequests(ScanRequest scanRequest)
		...
		GitPlatformMergeRequestService platformService =
				mergeRequestService.getMergeRequestService(scanRequest.getToolType());

		// Fetch new merge requests
		List<ScmMergeRequests> newMergeRequests =
				fetchNewMergeRequests(scanRequest, platformService, urlInfo, identifier);
		...
		List<ScmMergeRequests> updatedOpenMergeRequests =
				fetchUpdatesForOpenMergeRequests(scanRequest, platformService, urlInfo, identifier);
		log.info("Fetched {} updated open merge requests", updatedOpenMergeRequests.size());

		// Combine and deduplicate results
		List<ScmMergeRequests> combinedMergeRequests =
				combineAndDeduplicateMergeRequests(newMergeRequests, updatedOpenMergeRequests);
		log.info(
				"Combined total: {} unique merge requests after deduplication",
				combinedMergeRequests.size());

		return combinedMergeRequests;
	}

	private List<ScmMergeRequests> fetchNewMergeRequests(
		...
		log.info(
				"Fetching merge requests updated since: {} for identifier: {}",
				mergeRequestsSince,
				identifier);

		String token = formatToken(scanRequest);

		return platformService.fetchMergeRequests(
	...
	private List<ScmMergeRequests> fetchUpdatesForOpenMergeRequests(
			ScanRequest scanRequest,
			GitPlatformMergeRequestService platformService,
			GitUrlInfo urlInfo,
			String identifier)
		...
		log.debug(
				"Fetching MR updates since: {} for {} existing open MRs (based on updated date)",
				updatesSince,
		...
		// changes
		List<ScmMergeRequests> allRecentMRs =
	...
	private List<ScmMergeRequests> getExistingOpenMergeRequests(String identifier) {
		try {
			...
			log.warn(
					"Failed to fetch existing open merge requests for identifier {}: {}",
					identifier,
					e.getMessage());
			return List.of();
		}
	...
	/**
	 * Calculates the start date for fetching updates based on existing open MRs. Uses the oldest
	 * creation date among open MRs, with a reasonable minimum window.
	 */
	...
	private List<ScmMergeRequests> filterRelevantUpdates(
			List<ScmMergeRequests> allRecentMRs, List<ScmMergeRequests> existingOpenMRs) {
		// Create a set of external IDs from existing open MRs for efficient lookup
		Set<String> existingOpenMRIds =
				existingOpenMRs.stream().map(ScmMergeRequests::getExternalId).collect(Collectors.toSet());
	...
	/**
	 * Combines new merge requests and updated open merge requests, removing duplicates. Priority is
	 * given to the updated versions over new versions.
	 */
	private List<ScmMergeRequests> combineAndDeduplicateMergeRequests(
			List<ScmMergeRequests> newMergeRequests, List<ScmMergeRequests> updatedOpenMergeRequests) {
	...
	private LocalDateTime calculateMergeRequestsSince(ScanRequest scanRequest) {
		if (scanRequest.getLastScanFrom() != null && scanRequest.getLastScanFrom() != 0L) {
			...
			log.debug(
					"Using firstScanFrom ({} months) for merge requests (updated date filter): {}",
					firstScanFromMonths,
					mergeRequestsSince);
			return mergeRequestsSince;
		}
