...
/**
 * AzureRepoProcessorJobExecutor represents a class which holds all the configuration and Azure Repo
 * execution process.
 *
...
public class AzureRepoProcessorJobExecutor extends ProcessorJobExecutor<AzureRepoProcessor> {
	...
	@Autowired
	protected AzureRepoProcessorJobExecutor(
			TaskScheduler taskScheduler, // NOSONAR
			AzureRepoProcessorRepository azureRepoProcessorRepo,
			AzureRepoConfig azureRepoConfig,
			ProjectToolConfigRepository toolConfigRepository,
			AzureRepoRepository azureRepoRepository,
			AzureRepoClient azureRepoClient,
	...
	private boolean isProcessorItemExist(ProjectToolConfig tool, List<ProcessorItem> processorItems) {
		...
		for (ProcessorItem processorItem : processorItems) {
			...
			if (options.containsKey(AzureRepoConstants.SCM)
					&& options.containsKey(AzureRepoConstants.URL)
					&& options.containsKey(AzureRepoConstants.TOOL_BRANCH)
					&& options.containsKey(AzureRepoConstants.REPOSITORY_NAME)) {
				sb.append(options.get(AzureRepoConstants.SCM))
						.append(options.get(AzureRepoConstants.URL))
	...
	public boolean execute(AzureRepoProcessor processor) {
		...
		for (ProjectBasicConfig proBasicConfig : projectConfigList) {
			...
			List<ProcessorToolConnection> azureRepoInfo =
					processorToolConnectionService.findByToolAndBasicProjectConfigId(
							ProcessorConstants.AZUREREPO, proBasicConfig.getId());
		...
		MDC.put(
				"TotalAzureRepoProcessorJobExecutorTime",
			...
			for (ProcessorToolConnection entry : azureRepoInfo) {
				try {
					...
					if (azureRepo.getToolConfigId().equals(entry.getId())) {
						...
						MDC.put(
								"AzurerepoReposDataCollectionStarted",
								"Azurerepo Processor started collecting data for Url: "
										+ entry.getUrl()
										+ " and branch : "
										+ entry.getBranch());

						List<CommitDetails> commitDetailList =
								azureRepoClient.fetchAllCommits(azureRepo, firstTimeRun, entry, projectBasicConfig);
						if (projectBasicConfig.isSaveAssigneeDetails()
								&& !processorExecutionTraceLog.isLastEnableAssigneeToggleState()) {
						...
						List<CommitDetails> unsavedCommits =
								commitDetailList.stream()
										.filter(commit -> isNewCommit(azureRepo, commit))
						...
						MDC.put(
								"AzurereppoReposDataCollectionCompleted",
								"Azurerepo Processor collected data for Url: "
										+ entry.getUrl()
										+ " and branch : "
										+ entry.getBranch());
						reposCount++;
	...
	private int processMergeRequestData(
			List<AzureRepoModel> azurerepoRepos,
			List<ProcessorToolConnection> azureRepoInfo,
			int reposCount,
				...
				try {
					if (azureRepo.getToolConfigId().equals(entry.getId())) {
						...
						List<MergeRequests> mergeRequestsList =
								azureRepoClient.fetchAllMergeRequest(
										azureRepo, firstTimeRun, entry, proBasicConfig);
						if (proBasicConfig.isSaveAssigneeDetails()
								&& !processorExecutionTraceLog.isLastEnableAssigneeToggleState()) {
						...
						List<MergeRequests> unsavedMergeRequests =
								mergeRequestsList.stream()
										.filter(mergReq -> isNewMergeReq(azureRepo, mergReq))
										.collect(Collectors.toList());
	...
	private ProcessorExecutionTraceLog createTraceLog(String basicProjectConfigId) {
		...
		Optional<ProcessorExecutionTraceLog> existingTraceLogOptional =
				processorExecutionTraceLogRepository.findByProcessorNameAndBasicProjectConfigId(
						ProcessorConstants.AZUREREPO, basicProjectConfigId);
	...
	private void cacheRestClient(String cacheEndPoint, String cacheName) {
		...
		UriComponentsBuilder uriBuilder =
				UriComponentsBuilder.fromHttpUrl(azureRepoConfig.getCustomApiBaseUrl());
		uriBuilder.path("/");
	 ...
	 * @param processor
	 */
	private void cleanUnusedProcessorItem(
			List<ProjectToolConfig> tools, AzureRepoProcessor processor) {
		...
		if (CollectionUtils.isEmpty(tools)) {
			...
			CollectionUtils.emptyIfNull(azureRepoModels).stream()
					.forEach(
							item -> {
								boolean itemExists =
										tools.stream().anyMatch(t -> item.getProcessorId().equals(t.getId()));
								if (!itemExists) {
