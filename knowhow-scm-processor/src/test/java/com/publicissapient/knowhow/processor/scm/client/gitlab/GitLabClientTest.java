...
class GitLabClientTest {
	...
	@Mock private MergeRequestApi mergeRequestApi;

	@Mock private NotesApi notesApi;
	...
	void testGetGitLabClientFromRepoUrl_FallbackToDefault() throws Exception {
		GitLabClient spyClient = spy(gitLabClient);
		when(gitUrlParser.getGitLabApiBaseUrl(TEST_REPO_URL))
	...
	void testFetchCommits_Success() throws Exception {
		...
		when(commitsApi.getCommits(anyLong(), eq(TEST_BRANCH), any(), any(), anyInt()))
		...
		List<Commit> result =
				spyClient.fetchCommits(
						TEST_ORG, TEST_REPO, TEST_BRANCH, TEST_TOKEN, since, until, TEST_REPO_URL);

		assertNotNull(result);
		assertEquals(2, result.size());
		verify(rateLimitService, atLeastOnce())
				.checkRateLimit(eq("GitLab"), eq(TEST_TOKEN), anyString(), eq(TEST_API_URL));
	}
	...
	void testFetchCommits_WithNullDates_Success() throws Exception {
		...
		when(gitLabApi.getCommitsApi()).thenReturn(commitsApi);
		when(projectApi.getProject(TEST_ORG + "/" + TEST_REPO)).thenReturn(mockProject);
		when(commitsApi.getCommits(anyLong(), eq(TEST_BRANCH), isNull(), isNull(), anyInt()))
				.thenReturn(mockPager);

		List<Commit> result =
				spyClient.fetchCommits(
						TEST_ORG, TEST_REPO, TEST_BRANCH, TEST_TOKEN, null, null, TEST_REPO_URL);
	...
	//        when(mockPager.next()).thenReturn(commits);
	//
	//        doReturn(gitLabApi).when(spyClient).getGitLabClientFromRepoUrl(TEST_TOKEN,
	// TEST_REPO_URL);
	//        when(gitLabApi.getGitLabServerUrl()).thenReturn(TEST_API_URL);
	//        when(gitLabApi.getProjectApi()).thenReturn(projectApi);
	//        when(gitLabApi.getCommitsApi()).thenReturn(commitsApi);
	//        when(projectApi.getProject(TEST_ORG + "/" + TEST_REPO)).thenReturn(mockProject);
	//        when(commitsApi.getCommits(anyLong(), eq(TEST_BRANCH), any(), any(),
	// anyInt())).thenReturn(mockPager);
	//
	//        List<Commit> result = spyClient.fetchCommits(TEST_ORG, TEST_REPO, TEST_BRANCH,
	// TEST_TOKEN,
	...
	void testFetchCommits_GitLabApiException() throws Exception {
		...
		when(gitLabApi.getGitLabServerUrl()).thenReturn(TEST_API_URL);
		when(gitLabApi.getProjectApi()).thenReturn(projectApi);
		when(projectApi.getProject(TEST_ORG + "/" + TEST_REPO))
				.thenThrow(new GitLabApiException("API Error", 404));
