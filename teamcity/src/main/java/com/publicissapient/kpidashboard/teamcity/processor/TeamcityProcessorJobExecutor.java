...
public class TeamcityProcessorJobExecutor extends ProcessorJobExecutor<TeamcityProcessor> {
	...
	@Autowired private TeamcityProcessorRepository teamcityProcessorRepository;
	@Autowired private BuildRepository buildRepository;
	@Autowired private TeamcityConfig teamcityConfig;
	@Autowired private ProjectToolConfigRepository projectToolConfigRepository;
	@Autowired private TeamcityClientFactory teamcityClientFactory;
	 ...
	 *
	 * @param processor the teamcity processor instance
	 */
	@Override
	public boolean execute(TeamcityProcessor processor) {
		...
		for (ProjectBasicConfig proBasicConfig : projectConfigList) {
			List<ProcessorToolConnection> teamcityJobList =
					processorToolConnectionService.findByToolAndBasicProjectConfigId(
							ProcessorConstants.TEAMCITY, proBasicConfig.getId());
			...
			for (ProcessorToolConnection teamcityServer : teamcityJobList) {
				...
				try {
					...
					int updatedJobs =
							addNewBuilds(
									savedTotalBuilds, buildsByJob, teamcityServer, processor.getId(), proBasicConfig);
					count += updatedJobs;
	 ...
	 *
	 * @param teamcityServer teamcityServer
	 * @param exception exception
	 */
	private void isClientException(
			ProcessorToolConnection teamcityServer, RestClientException exception) {
		if (exception instanceof HttpClientErrorException
			...
			processorToolConnectionService.updateBreakingConnection(
					teamcityServer.getConnectionId(), errMsg);
		}
	}
	...
	private ProcessorExecutionTraceLog createTraceLog(String basicProjectConfigId) {
		...
		Optional<ProcessorExecutionTraceLog> existingTraceLogOptional =
				processorExecutionTraceLogRepository.findByProcessorNameAndBasicProjectConfigId(
						ProcessorConstants.TEAMCITY, basicProjectConfigId);
	...
	private int addNewBuilds(
		...
		builds.sort(
				(Build b1, Build b2) -> Integer.valueOf(b1.getNumber()) - Integer.valueOf(b2.getNumber()));
		for (Build buildSummary : builds) {
			Build buildData =
					buildRepository.findByProjectToolConfigIdAndNumber(
							teamcityServer.getId(), buildSummary.getNumber());
			if (buildData == null) {
				Build build =
						teamcityClient.getBuildDetails(
								buildSummary.getBuildUrl(),
								teamcityServer.getUrl(),
								teamcityServer,
								proBasicConfig);
				if (build != null) {
