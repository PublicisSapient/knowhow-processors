...
public class SprintReportTaskletTest {
	...
	public void testExecute_Success() throws Exception {
		// Arrange
		when(fetchProjectConfiguration.fetchConfigurationBasedOnSprintId(sprintId))
		...
		for (String boardId : originalBoardIds) {
			when(fetchSprintReport.getSprints(projectConfFieldMapping, boardId))
					.thenReturn(sprintDetailsList);
			when(fetchSprintReport.fetchSprints(
							eq(projectConfFieldMapping), any(), eq(true), any(ObjectId.class)))
		...
		verify(fetchSprintReport, times(originalBoardIds.size()))
				.fetchSprints(any(), any(), eq(true), any(ObjectId.class));
	...
	public void testExecute_NoMatchingSprintInDetailsList() throws Exception {
		...
		verify(fetchSprintReport, times(originalBoardIds.size()))
				.fetchSprints(any(), any(), anyBoolean(), any());
		verify(sprintRepository, times(originalBoardIds.size())).saveAll(any());
	}
