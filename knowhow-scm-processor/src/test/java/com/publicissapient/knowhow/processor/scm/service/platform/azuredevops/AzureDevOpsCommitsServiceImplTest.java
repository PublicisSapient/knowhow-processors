...
class AzureDevOpsCommitsServiceImplTest {
	...
	@InjectMocks private AzureDevOpsCommitsServiceImpl service;
	...
	void setUp() {
		...
		when(gitUrlInfo.getOriginalUrl())
				.thenReturn("https://dev.azure.com/testOrg/testProject/_git/testRepo");
	...
	void fetchCommits_Success() throws Exception {
		...
		when(azureDevOpsClient.fetchCommits(
		...
		verify(rateLimitService)
				.checkRateLimit(eq("AzureDevOps"), eq(token), eq("testRepo"), anyString());
	}
	...
	void fetchCommits_RateLimitCheck() throws Exception {
		...
		verify(rateLimitService)
				.checkRateLimit("AzureDevOps", token, "testRepo", gitUrlInfo.getOriginalUrl());
	}
	...
	void fetchCommits_WithNullProject() throws Exception {
		...
		GitCommitRef azureCommit = createMockGitCommitRef();
		when(azureDevOpsClient.fetchCommits(
						eq("testOrg"), eq("testRepo"), eq("testRepo"), anyString(), anyString(), any(), any()))
				.thenReturn(Arrays.asList(azureCommit));
	...
	private GitCommitRef createMockGitCommitRef() {
		...
		when(commit.getUrl())
				.thenReturn("https://dev.azure.com/testOrg/testProject/_git/testRepo/commit/abc123");
