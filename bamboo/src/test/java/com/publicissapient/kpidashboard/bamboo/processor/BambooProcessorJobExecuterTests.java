...
public class BambooProcessorJobExecuterTests {
	...
	public void init() {
		...
		BAMBOOSAMPLESERVER.setPassword(SecureStringUtil.generateRandomPassword(6));
	...
	public void collectJobsAdded() throws MalformedURLException, ParseException {
		try {
			...
			// when(processorToolConnectionService.findByToolAndBasicProjectConfigId(any(),
			// any()))
	...
	public void collectTwoJobsJobsAddedRandomOrder() throws MalformedURLException, ParseException {
		try {
			...
			// when(bambooClient.getBuildDetailsFromServer(any(), any(),
			// any())).thenReturn(build);
			//
			// when(projectConfigRepository.findActiveProjects(anyBoolean())).thenReturn(projectConfigList);
			when(deploymentRepository.findAll()).thenReturn(deploymentList);
			// when(processorToolConnectionService.findByToolAndBasicProjectConfigId(any(),
	...
	public void testAddNewBuildsInfoToDb_buildNotNull_success() throws Exception {
		try {
			...
			Assert.assertEquals("Exception is: ", EXCEPTION, cause.getMessage());
		}
	}
