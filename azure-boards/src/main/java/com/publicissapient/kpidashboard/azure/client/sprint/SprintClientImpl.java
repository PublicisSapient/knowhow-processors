...
public class SprintClientImpl implements SprintClient {
	...
	@Autowired private AzureSprintReportLogService azureSprintReportLogService;

	/**
	 * all sprint issues saved based on status configure field mapping field status closed sprint
	 * issues will be not changes after saved sprint details once active sprint issues bucket changes
	 * based on comparison of dbSprintDetails vs fetched issues future sprint issues only changes
	 * while
	 *
	 * @param projectConfig
	 ...
	 */
	@Override
	public void prepareSprintReport(
		...
		try {
			...
			sprintDetailsSet.forEach(
					fetchedSprintDetails -> {
						SprintDetails dbSprintDetails =
								sprintRepository.findBySprintID(fetchedSprintDetails.getSprintID());
						if (Objects.isNull(dbSprintDetails)) {
							...
							log.info(
									"fetched sprint Name -> {} , sprint state -> {} ",
									fetchedSprintDetails.getSprintName(),
									fetchedSprintDetails.getState());
							List<SprintIssue> fetchedSprintWiseIssues =
									fetchAndPrepareSprintIssue(
											azureAdapter, azureServer, projectConfig, fetchedSprintDetails);
							Set<SprintIssue> toBeSavedCompletedIssues = new java.util.HashSet<>();
							Set<SprintIssue> toBeSavedNotCompletedIssues = new HashSet<>();
							Set<SprintIssue> toBeSavedTotalIssues = new HashSet<>();
							prepareCompletedAndNotCompletedSprintIssue(
									completedIssuesStatus,
									fetchedSprintWiseIssues,
									toBeSavedCompletedIssues,
									toBeSavedNotCompletedIssues,
									toBeSavedTotalIssues);
							...
							log.debug(
									"saved sprint Id -> {} , toBeSavedCompletedIssues -> {} , toBeSavedNotCompletedIssues -> {}",
									fetchedSprintDetails.getSprintID(),
									getIssuesIdList(toBeSavedCompletedIssues),
									getIssuesIdList(toBeSavedNotCompletedIssues));

						} else {
							// fetched and db sprint is active then issues bucket compare as per
							if (fetchedSprintDetails
											.getState()
											.equalsIgnoreCase(SprintDetails.SPRINT_STATE_ACTIVE)
									&& dbSprintDetails
											.getState()
											.equalsIgnoreCase(SprintDetails.SPRINT_STATE_ACTIVE)) {
								if (Boolean.TRUE.equals(
										projectConfig.getProjectToolConfig().getAzureRefreshActiveSprintReport())) {
									// if azurerefreshSprintReport is active- then treat it just like the case
									// when in db sprint is future and from server it is active.
									overrideDBSprintReport(
											projectConfig,
											azureAdapter,
											azureServer,
											fetchedSprintDetails,
									...
									azureSprintReportLogService.saveSprintRefreshLog(
											fetchedSprintDetails,
											projectConfig.getBasicProjectConfigId(),
											System.currentTimeMillis(),
											projectConfig
													.getProjectToolConfig()
													.getAzureRefreshActiveSprintReportUpdatedBy());
									...
									log.debug(
											...
											dbSprintDetails.getAddedIssues(),
									...
									prepareAddedSprintIssues(
											dbSprintDetails, fetchedSprintWiseIssues, toBeSavedAddedIssues);
									preparePuntedSprintIssues(
											dbSprintDetails, fetchedSprintWiseIssues, toBeSavedPuntedIssues);
									...
									fetchedSprintDetails.setBasicProjectConfigId(
											projectConfig.getBasicProjectConfigId());
									fetchedSprintDetails.setProcessorId(azureProcessorId);
							...
							if (fetchedSprintDetails
									...
									&& dbSprintDetails
											.getState()
											.equalsIgnoreCase(SprintDetails.SPRINT_STATE_FUTURE)) {
			...
			log.error("Error while preparing sprint report -> {}", e.getMessage());
			throw new SprintReportException("Error preparing sprint report", e);
		}
	}

	private void overrideDBSprintReport(
			ProjectConfFieldMapping projectConfig,
			AzureAdapter azureAdapter,
			AzureServer azureServer,
			SprintDetails fetchedSprintDetails,
			List<String> completedIssuesStatus,
			ObjectId azureProcessorId,
			SprintDetails dbSprintDetails) {
		List<SprintIssue> fetchedSprintWiseIssues =
				fetchAndPrepareSprintIssue(azureAdapter, azureServer, projectConfig, fetchedSprintDetails);
	...
	/**
	 * based on field mapping completed issues status divided into completed and not completed issues
	 * bucket
	 *
	 ...
	 */
	private void prepareCompletedAndNotCompletedSprintIssue(
			List<String> completedIssuesStatus,
			List<SprintIssue> fetchedSprintWiseIssues,
			Set<SprintIssue> toBeSavedCompletedIssues,
			Set<SprintIssue> toBeSavedNotCompletedIssues,
			Set<SprintIssue> toBeSavedTotalIssues) {

		fetchedSprintWiseIssues.forEach(
				sprintIssue -> {
	...
	/**
	 * if any fetched issues not found in db total issues then count into removed issues for active
	 * sprint
	 *
	 ...
	 */
	private void preparePuntedSprintIssues(
			SprintDetails dbSprintDetails,
			List<SprintIssue> fetchedSprintWiseIssues,
			Set<SprintIssue> toBeSavedPuntedIssues) {
		if (CollectionUtils.isNotEmpty(dbSprintDetails.getTotalIssues())) {
			List<String> fetchedTotalIssues =
					fetchedSprintWiseIssues.stream()
			...
			Set<SprintIssue> removedIssues =
					dbSprintDetails.getTotalIssues().stream()
							.filter(Objects::nonNull)
			...
			log.debug(
					"Active Sprint Name -> {} , fetched total issues -> {} , removed issues -> {} ",
					dbSprintDetails.getSprintName(),
					fetchedTotalIssues,
					getIssuesIdList(removedIssues));
		}
	}

	/**
	 * if db total issues and fetched total issues diff is added issue for active sprint
	 *
	 * @param dbSprintDetails
	 * @param fetchedSprintWiseIssues
	 * @param toBeSavedAddedIssues
	 */
	private void prepareAddedSprintIssues(
			SprintDetails dbSprintDetails,
			List<SprintIssue> fetchedSprintWiseIssues,
			Set<String> toBeSavedAddedIssues) {
		if (CollectionUtils.isNotEmpty(dbSprintDetails.getTotalIssues())) {
			List<String> dbTotalIssues =
					dbSprintDetails.getTotalIssues().stream()
							.filter(Objects::nonNull)
							.map(SprintIssue::getNumber)
							.distinct()
			...
			Set<String> addedIssues =
					fetchedSprintWiseIssues.stream()
							.filter(Objects::nonNull)
							.filter(sprintIssue -> !dbTotalIssues.contains(sprintIssue.getNumber()))
							.map(SprintIssue::getNumber)
							.collect(Collectors.toSet());
			...
			if (CollectionUtils.isNotEmpty(dbSprintDetails.getPuntedIssues())
				...
				Set<SprintIssue> puntedAndAddedWithInSprint =
						dbPuntedIssues.stream()
								.filter(Objects::nonNull)
				...
				log.debug(
						"Sprint Id -> {} , puntedAndAddedIds -> {}",
						dbSprintDetails.getSprintID(),
						puntedAndAddedIds);
			}

			log.debug(
					"Active Sprint Name -> {} , db total issues -> {} , added issues -> {} ",
					dbSprintDetails.getSprintName(),
					dbTotalIssues,
					addedIssues);
		}
	...
	/**
	 * if field mapping completedIssuesStatus field is changes then existing sprint report issues will
	 * be shuffled as per status
	 *
	 * @param projectConfig
	 * @param completedIssuesStatus
	 */
	private void iterationStatusUpdateSPIssuesShuffle(
			ProjectConfFieldMapping projectConfig, List<String> completedIssuesStatus) {
		...
		if (projectToolConfig.isAzureIterationStatusFieldUpdate()
			...
			List<SprintDetails> dbSprintDetailsList =
					sprintRepository.findByBasicProjectConfigId(projectConfig.getBasicProjectConfigId());
			dbSprintDetailsList.stream()
					.forEach(
							sprintDetails -> {
			...
			log.debug(
					"sprint issues shuffle based on completedIssuesStatus field update -> ",
	 ...
	 * @return
	 */
	private List<SprintIssue> fetchAndPrepareSprintIssue(
			AzureAdapter azureAdapter,
			AzureServer azureServer,
			ProjectConfFieldMapping projectConfig,
			SprintDetails fetchedSprintDetails) {
		List<String> sprintWiseItemIdList =
				azureAdapter.getIssuesBySprint(azureServer, fetchedSprintDetails.getOriginalSprintId());
		List<String> sprintWiseIssueList = new ArrayList<>();
		List<SprintIssue> sprintIssueList = new ArrayList<>();
		sprintWiseItemIdList.stream()
				.forEach(id -> sprintWiseIssueList.add(getModifiedIssueId(projectConfig, id)));
		List<JiraIssue> jiraIssueList =
				jiraIssueRepository.findByNumberInAndBasicProjectConfigId(
						sprintWiseIssueList, projectConfig.getBasicProjectConfigId().toString());
		if (CollectionUtils.isNotEmpty(jiraIssueList)) {
			jiraIssueList.stream()
					.forEach(jiraIssue -> sprintIssueList.add(setSprintIssueDetails(jiraIssue)));
		}
		log.info(
				"fetched sprint Id -> {} , Issues -> {} ",
				fetchedSprintDetails.getSprintID(),
				sprintWiseItemIdList);
