...
public class AzureDevOpsCommitsServiceImpl implements GitPlatformCommitsService {
	...
	@Autowired
	public AzureDevOpsCommitsServiceImpl(
			AzureDevOpsClient azureDevOpsClient, RateLimitService rateLimitService) {
	...
	public List<ScmCommits> fetchCommits(
		...
		try {
			log.info(
					"Fetching commits for Azure DevOps repository: {}/{}/{}",
			...
			rateLimitService.checkRateLimit(
					PLATFORM_NAME, token, gitUrlInfo.getRepositoryName(), gitUrlInfo.getOriginalUrl());

			RepositoryDetails repoDetails = extractRepositoryDetails(gitUrlInfo);

			List<GitCommitRef> azureCommits =
					azureDevOpsClient.fetchCommits(
							repoDetails.organization(),
							repoDetails.project(),
			...
			List<ScmCommits> commitDetails =
					convertAzureCommitsToScmCommits(azureCommits, toolConfigId, repoDetails, branchName);

			log.info(
					"Successfully converted {} Azure DevOps commits to domain objects", commitDetails.size());
	...
	private RepositoryDetails extractRepositoryDetails(GitUrlParser.GitUrlInfo gitUrlInfo) {
		...
		String project =
				gitUrlInfo.getProject() != null ? gitUrlInfo.getProject() : gitUrlInfo.getRepositoryName();
		String repository = gitUrlInfo.getRepositoryName();
		return new RepositoryDetails(organization, project, repository);
	}

	private List<ScmCommits> convertAzureCommitsToScmCommits(
			List<GitCommitRef> azureCommits,
			String toolConfigId,
		...
		for (GitCommitRef azureCommit : azureCommits) {
			try {
				...
				log.warn(
						"Failed to convert Azure DevOps commit {}: {}",
						azureCommit.getCommitId(),
						e.getMessage());
			}
		}
