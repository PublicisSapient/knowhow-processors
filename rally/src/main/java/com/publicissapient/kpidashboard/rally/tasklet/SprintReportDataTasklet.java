...
public class SprintReportDataTasklet implements Tasklet {
	...
	public RepeatStatus execute(StepContribution sc, ChunkContext cc) throws Exception {
		...
		List<SprintDetails> sprintDetailsList =
				sprintRepository.findByBasicProjectConfigId(projConfFieldMapping.getBasicProjectConfigId());
		List<JiraIssue> jiraIssueList =
				jiraIssueRepository.findByBasicProjectConfigIdIn(
						projConfFieldMapping.getBasicProjectConfigId().toString());
		...
		if (CollectionUtils.isNotEmpty(sprintDetailsList)) {
			sprintDataProcessor.processSprintReportData(
					sprintDetailsList, jiraIssueCustomHistoryList, jiraIssueList);
