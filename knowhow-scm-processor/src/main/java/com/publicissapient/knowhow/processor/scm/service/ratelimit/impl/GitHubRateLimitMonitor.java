 ...
 *
 * <p>This monitor uses the GitHub API to check current rate limit status and validates against
...
public class GitHubRateLimitMonitor implements RateLimitMonitor {
	...
	private GitHub createGitHubClient(String token) throws IOException {
		if (token != null && !token.isEmpty()) {
			return new GitHubBuilder().withEndpoint(githubApiUrl).withOAuthToken(token).build();
		} else {
