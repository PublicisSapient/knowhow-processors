...
public class ReleaseFrequencyControllerTest {
	...
	@Mock private ReleaseDataService releaseDataService;

	@InjectMocks private ReleaseFrequencyController releaseFrequencyController;
	...
	@Test
	@DisplayName(
			"Should return success response when fetchReleaseData is called with valid project ID")
	...
	@Test
	@DisplayName(
			"Should return error response when fetchReleaseData is called with invalid project ID")
	...
	void testFetchReleaseData_MissingParameter() throws Exception {
		// Act & Assert
		mockMvc
				.perform(get("/rally/release/fetch-data").contentType(MediaType.APPLICATION_JSON))
