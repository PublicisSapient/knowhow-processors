...
public class AzureDevOpsMergeRequestServiceImpl implements GitPlatformMergeRequestService {
    ...
    public AzureDevOpsMergeRequestServiceImpl(AzureDevOpsClient azureDevOpsClient, AzureDevOpsCommonHelper commonHelper) {
    ...
    public List<ScmMergeRequests> fetchMergeRequests(String toolConfigId, GitUrlParser.GitUrlInfo gitUrlInfo,
        ...
        try {
            ...
            List<GitPullRequest> azurePRs = azureDevOpsClient.fetchPullRequests(
                    gitUrlInfo.getOwner(), gitUrlInfo.getProject(), gitUrlInfo.getRepositoryName(), token, since, branchName);

            List<ScmMergeRequests> mergeRequests = new ArrayList<>();
            for (GitPullRequest azPr : azurePRs) {
                try {
                    ScmMergeRequests mergeRequest = convertToMergeRequest(azPr, toolConfigId, gitUrlInfo.getOwner(),
    ...
    private ScmMergeRequests convertToMergeRequest(GitPullRequest azPr, String toolConfigId, String owner, String repository) {
