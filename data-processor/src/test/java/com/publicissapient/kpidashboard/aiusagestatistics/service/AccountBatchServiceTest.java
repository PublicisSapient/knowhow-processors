...
class AccountBatchServiceTest {
	...
	@Mock private AccountHierarchyRepository repository;
	@InjectMocks private AccountBatchService service;
	...
	void initializeBatchProcessingParametersForTheNextProcess_loadsAccountsAndSorts() {
		List<AccountHierarchy> accounts =
				List.of(createAccount("2", "AccountB"), createAccount("1", "AccountA"));
	...
	void getNextAccount_returnsAccountsOneByOne() {
		List<AccountHierarchy> accounts =
				List.of(createAccount("1", "Account1"), createAccount("2", "Account2"));
		when(repository.findDistinctByLabel("acc")).thenReturn(accounts);
