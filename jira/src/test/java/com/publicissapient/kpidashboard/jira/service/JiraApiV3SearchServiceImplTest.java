...
public class JiraApiV3SearchServiceImplTest {
	...
	@InjectMocks private JiraApiV3SearchServiceImpl jiraApiV3SearchService;
	...
	public void setUp() {
		...
		jiraToolConfig = JiraToolConfig.builder().connection(Optional.of(connection)).build();
	}
	...
	public void testSearchJql_ShortJql_Success() throws Exception {
		...
		try (MockedStatic<Unirest> unirestMock = mockStatic(Unirest.class)) {
			...
			JiraSearchResponse result =
					jiraApiV3SearchService.searchJql(shortJql, 50, fields, "test", jiraToolConfig);
	...
	public void testSearchJql_NoConnection_ThrowsException() {
		jiraToolConfig = JiraToolConfig.builder().connection(Optional.empty()).build();
		...
		try (MockedStatic<Unirest> unirestMock = mockStatic(Unirest.class)) {
			...
			JiraSearchResponse result =
					jiraApiV3SearchService.searchJql(
							jql, 50, Set.of("summary", "status"), "test", jiraToolConfig);
	...
	public void testSearchJql_NullJql() throws Exception {
		...
		try (MockedStatic<Unirest> unirestMock = mockStatic(Unirest.class)) {
			...
			JiraSearchResponse result =
					jiraApiV3SearchService.searchJql(
							"In [Story]", 50, Set.of("summary", "status"), "test", jiraToolConfig);
