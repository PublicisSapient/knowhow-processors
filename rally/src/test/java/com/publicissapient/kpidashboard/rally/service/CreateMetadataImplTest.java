...
class CreateMetadataImplTest {
	...
	void testCollectMetadata_WhenMetadataNotPresent() {
		...
		// Verify interactions
		verify(boardMetadataRepository, times(1))
				.deleteByProjectBasicConfigId(projectConfig.getBasicProjectConfigId());
	...
	void testCollectMetadata_WhenMetadataAlreadyPresent() {
		when(boardMetadataRepository.findByProjectBasicConfigId(
	...
	void testFetchTypeDefinitions_Success() throws JsonProcessingException {
		...
		RallyTypeDefinitionResponse.QueryResult queryResult =
				new RallyTypeDefinitionResponse.QueryResult();

		RallyTypeDefinitionResponse.TypeDefinition type1 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		type1.setRefObjectName("HierarchicalRequirement");
		RallyTypeDefinitionResponse.TypeDefinition type2 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		type2.setRefObjectName("Defect");
		RallyTypeDefinitionResponse.TypeDefinition type3 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		type3.setRefObjectName("Task");
		RallyTypeDefinitionResponse.TypeDefinition type4 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		type4.setRefObjectName("TestCase");
		RallyTypeDefinitionResponse.TypeDefinition type5 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		type5.setRefObjectName("DefectSuite");
		RallyTypeDefinitionResponse.TypeDefinition type6 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		...
		ResponseEntity<RallyTypeDefinitionResponse> responseEntity =
				new ResponseEntity<>(response, HttpStatus.OK);
		when(rallyRestClient.get(
						eq(typesUrl), eq(projectConfig), eq(RallyTypeDefinitionResponse.class)))
		...
		verify(rallyRestClient, times(1))
				.get(eq(typesUrl), eq(projectConfig), eq(RallyTypeDefinitionResponse.class));
	}
	...
	void testFetchAllowedValues_Success() throws JsonProcessingException {
		...
		RallyAllowedValuesResponse.QueryResult queryResult =
				new RallyAllowedValuesResponse.QueryResult();
		...
		queryResult.setResults(
				Arrays.asList(value1, value2, value3, value4, value5, value6, value7, value8, value9));
		queryResult.setErrors(Collections.emptyList());
		response.setQueryResult(queryResult);

		ResponseEntity<RallyAllowedValuesResponse> responseEntity =
				new ResponseEntity<>(response, HttpStatus.OK);
		when(rallyRestClient.get(
						eq(allowedValuesUrl), eq(projectConfig), eq(RallyAllowedValuesResponse.class)))
				.thenReturn(responseEntity);
		...
		verify(rallyRestClient, times(1))
				.get(eq(allowedValuesUrl), eq(projectConfig), eq(RallyAllowedValuesResponse.class));
	}
	...
	void testEvictCaches() {
		...
		verify(rallyProcessorCacheEvictor, times(5))
				.evictCache(eq(CommonConstant.CACHE_CLEAR_ENDPOINT), anyString());
	}
	...
	void testGetMetadataValues_WithValidQueryResult() {
		...
		RallyTypeDefinitionResponse.QueryResult queryResult =
				new RallyTypeDefinitionResponse.QueryResult();
		RallyTypeDefinitionResponse.TypeDefinition result1 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		result1.setRefObjectName("HierarchicalRequirement");
		RallyTypeDefinitionResponse.TypeDefinition result2 =
				new RallyTypeDefinitionResponse.TypeDefinition();
		result2.setRefObjectName("Defect");
		RallyTypeDefinitionResponse.TypeDefinition result3 =
				new RallyTypeDefinitionResponse.TypeDefinition();
	...
	void testCreateBoardMetadata() {
		...
		// Verify
		ArgumentCaptor<BoardMetadata> boardMetadataCaptor =
				ArgumentCaptor.forClass(BoardMetadata.class);
		...
		assertEquals(
				projectConfig.getProjectToolConfig().getId(), savedMetadata.getProjectToolConfigId());
		assertEquals(
				projectConfig.getProjectToolConfig().getOriginalTemplateCode(),
				savedMetadata.getMetadataTemplateCode());
	...
	void testMapFieldMapping() {
		...
		assertEquals(
				projectConfig.getBasicProjectConfigId(), savedFieldMapping.getBasicProjectConfigId());
		assertEquals(
				projectConfig.getProjectToolConfig().getId(), savedFieldMapping.getProjectToolConfigId());
		...
		assertArrayEquals(
				new String[] {"HierarchicalRequirement", "Defect", "Task"},
				savedFieldMapping.getJiraIssueTypeNames());
