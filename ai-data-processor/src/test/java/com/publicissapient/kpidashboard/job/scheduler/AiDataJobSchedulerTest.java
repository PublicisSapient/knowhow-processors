...
class AiDataJobSchedulerTest {
	...
	@Mock private JobOrchestrator jobOrchestrator;

	@Mock private ThreadPoolTaskScheduler threadPoolTaskScheduler;

	@Mock private JobStrategy jobStrategyWithScheduling;

	@Mock private JobStrategy jobStrategyWithoutScheduling;

	@Mock private SchedulingConfig schedulingConfig;

	@InjectMocks private AiDataJobScheduler aiDataJobScheduler;
	...
	void when_ScheduleAllJobsWithJobsHavingSchedulingConfig_Then_SchedulesJobsWithCronTrigger() {
		...
		when(jobStrategyWithScheduling.getSchedulingConfig())
				...
				.thenReturn(Optional.of(schedulingConfig));
		when(schedulingConfig.getCron()).thenReturn(cronExpression1).thenReturn(cronExpression2);
		...
		verify(threadPoolTaskScheduler, times(2))
				.schedule(runnableCaptor.capture(), cronTriggerCaptor.capture());
