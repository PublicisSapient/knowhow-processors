...
public class SprintDataProcessorImpl implements SprintDataProcessor {
	...
	@Override
	public void processSprintReportData(
	...
	private Set<SprintDetails> createSprintDetails(
	...
	private static void initializeSprintDetails(
			List<JiraIssue> jiraIssueList,
			List<JiraIssueCustomHistory> jiraIssueCustomHistoryList,
						...
						.collect(
								Collectors.toMap(
										...
										(existing, replacement) ->
												existing // Keep the first occurrence when duplicates found
										));

		Map<String, Set<JiraIssue>> issuesBySprintName =
				jiraIssueList.stream()
		...
		for (SprintDetails sprintDetails : sprintDetailsList) {
			...
			Set<SprintIssue> totalIssues =
					convertToSprintIssues(issuesBySprintName.getOrDefault(sprintName, new HashSet<>()));

			Pair<Set<SprintIssue>, Set<SprintIssue>> addedAndRemovedIssues =
					processSprintHistory(jiraIssueCustomHistoryList, jiraIssueMap, sprintName);
			...
			setAddedAndRemovedIssues(
					sprintDetails,
					addedIssues.stream().map(SprintIssue::getNumber).collect(Collectors.toSet()),
					removedIssues);
	...
	private static void setBasicSprintDetails(
			Iteration iteration,
			ProjectConfFieldMapping projectConfig,
			ObjectId processorId,
		...
		else sprintDetails.setState(SprintDetails.SPRINT_STATE_FUTURE);
	}
	...
	private static Pair<Set<SprintIssue>, Set<SprintIssue>> processSprintHistory(
		...
		historyList.stream()
				.filter(history -> history.getSprintUpdationLog() != null)
				.forEach(
						history ->
								history
										.getSprintUpdationLog()
										.forEach(
												log -> {
	...
	private static void setAddedAndRemovedIssues(
			SprintDetails sprintDetails, Set<String> addedIssues, Set<SprintIssue> removedIssues) {
	...
	private static void separateAndSetIssuesByCompletionStatus(
			SprintDetails sprintDetails, Set<SprintIssue> totalIssues) {
