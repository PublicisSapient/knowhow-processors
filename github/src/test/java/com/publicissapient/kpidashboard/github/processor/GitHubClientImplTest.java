...
public class GitHubClientImplTest {
	@InjectMocks GitHubClientImpl gitHubClient;
	@Mock private GitHubConfig gitLabConfig;
	...
	public void testFetchCommits() throws Exception {
		String restURI =
				"https://test.com/repos/username/repositiryname/commits?per_page=100&sha=develop";
		...
		List<CommitDetails> commits =
				gitHubClient.fetchAllCommits(
						new GitHubProcessorItem(), true, getToolConnection(), new ProjectBasicConfig());
	...
	public void testMergeRequests() throws Exception {
		String restURI =
				"https://test.com/repos/username/repositiryname/pulls?per_page=100&state=all&base=develop";
		...
		doReturn(new ResponseEntity<>(serverResponse, HttpStatus.OK))
				...
				.exchange(
						...
						ArgumentMatchers.any(HttpEntity.class),
						ArgumentMatchers.eq(String.class));
		List<MergeRequests> mergeRequests =
