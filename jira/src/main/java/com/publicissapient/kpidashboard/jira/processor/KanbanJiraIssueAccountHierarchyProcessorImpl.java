...
@Slf4j
@Service
public class KanbanJiraIssueAccountHierarchyProcessorImpl
		implements KanbanJiraIssueAccountHierarchyProcessor {
	...
	@Override
	public Set<ProjectHierarchy> createKanbanAccountHierarchy(
			KanbanJiraIssue kanbanJiraIssue, ProjectConfFieldMapping projectConfig) {
		log.info(
				"Creating kanban_account_hierarchy for the project : {}", projectConfig.getProjectName());
		...
		if (StringUtils.isNotBlank(kanbanJiraIssue.getProjectName())) {
			...
			additionalFiltersHierarchies.forEach(
					accountHierarchy ->
							accHierarchyToSave(accountHierarchy, existingHierarchy, accHierarchyToSave));
		}
