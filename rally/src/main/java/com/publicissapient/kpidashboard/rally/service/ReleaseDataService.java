...
public class ReleaseDataService {
	...
	public ResponseEntity<ReleaseDataResponse> fetchAndProcessReleaseData(String projectId) {
		try {
			...
			if (toolConfig == null) {
				return ResponseEntity.status(HttpStatus.NOT_FOUND)
						.body(
								ReleaseDataResponse.error(
										"Rally tool configuration not found for project: " + projectId));
			}
			...
			return ResponseEntity.ok(
					ReleaseDataResponse.success(
							"Successfully fetched and stored release data for project "
									+ projectConfig.getProjectName()));

		} catch (IllegalArgumentException e) {
			log.error("Invalid argument while fetching release data", e);
			return ResponseEntity.badRequest().body(ReleaseDataResponse.error(e.getMessage()));
		} catch (IOException | ParseException e) {
			log.error("Error fetching release data from Rally", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(
							ReleaseDataResponse.error(
									"An error occurred while fetching release data from Rally: " + e.getMessage()));
		}
	}
	...
	private ProjectToolConfig findRallyToolConfig(ProjectBasicConfig projectConfig) {
		List<ProjectToolConfig> toolConfigs =
				projectToolConfigRepository.findByToolNameAndBasicProjectConfigId(
						RALLY_TOOL_NAME, projectConfig.getId());
	 ...
	 */
	private ProjectConfFieldMapping createFieldMapping(
			ProjectBasicConfig projectConfig, ProjectToolConfig toolConfig) {
		...
		log.info(
				"Initialized ProjectConfFieldMapping with projectBasicConfig ID: {} and projectToolConfig ID: {}",
				projectConfig.getId(),
				toolConfig.getId());
