...
class GitHubRateLimitMonitorTest {
	...
	@InjectMocks private GitHubRateLimitMonitor gitHubRateLimitMonitor;

	@Mock private GitHub github;

	@Mock private GHRateLimit ghRateLimit;

	@Mock private GHRateLimit.Record coreRecord;
	...
	void testCheckRateLimit_Success() throws IOException {
		try (MockedConstruction<GitHubBuilder> mockedBuilder =
				mockConstruction(
						GitHubBuilder.class,
						(mock, context) -> {
	...
	void testCheckRateLimit_ThrowsIOException() {
		try (MockedConstruction<GitHubBuilder> mockedBuilder =
			...
			assertThrows(
					IOException.class,
					() -> gitHubRateLimitMonitor.checkRateLimit(TEST_TOKEN, TEST_BASE_URL));
		}
	}
