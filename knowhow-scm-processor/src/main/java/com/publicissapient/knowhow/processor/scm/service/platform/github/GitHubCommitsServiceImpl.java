...
public class GitHubCommitsServiceImpl implements GitPlatformCommitsService {
	...
	public List<ScmCommits> fetchCommits(
		...
		try {
			log.info(
					"Fetching commits for GitHub repository: {}/{}",
					gitUrlInfo.getOwner(),
			...
			List<GHCommit> ghCommits =
					gitHubClient.fetchCommits(
							owner, gitUrlInfo.getRepositoryName(), branchName, token, since, until);
			...
			log.error(
					"Failed to fetch commits from GitHub repository {}/{}: {}",
					gitUrlInfo.getOwner(),
					gitUrlInfo.getRepositoryName(),
					e.getMessage());
	...
	private ScmCommits convertToCommit(
			GHCommit ghCommit, String toolConfigId, String owner, String repository) throws IOException {
		...
		builder
				.addedLines(diffStats.getAddedLines())
	...
	private void setCommitAuthor(ScmCommits.ScmCommitsBuilder builder, GHCommit ghCommit)
			throws IOException {
	...
	private GitHubDiffStats extractDiffStats(GHCommit ghCommit) {
		try {
			...
			return new GitHubDiffStats(
					totalAdditions, totalDeletions, totalChanges, files.size(), fileChanges);

		} catch (Exception e) {
			log.warn(
					"Failed to extract diff stats from commit {}: {}", ghCommit.getSHA1(), e.getMessage());
