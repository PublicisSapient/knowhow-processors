...
/**
 * GitHubProcessorJobExecutor represents a class which holds all the configuration and GitHub
 * execution process.
 *
 * @see GitHubProcessor
 */
...
public class GitHubProcessorJobExecutor extends ProcessorJobExecutor<GitHubProcessor> {
	...
	@Autowired private GitHubProcessorRepository gitHubProcessorRepository;

	@Autowired private GitHubConfig gitHubConfig;

	@Autowired private GitHubProcessorItemRepository gitHubProcessorItemRepository;
	 ...
	 */
	private GitHubProcessorItem createProcessorItem(
			ProcessorToolConnection tool, ObjectId processorId) {
	...
	public boolean execute(GitHubProcessor processor) {
		...
		for (ProjectBasicConfig proBasicConfig : projectConfigList) {
			List<ProcessorToolConnection> githubJobsFromConfig =
					processorToolConnectionService.findByToolAndBasicProjectConfigId(
							ProcessorConstants.GITHUB, proBasicConfig.getId());
			for (ProcessorToolConnection tool : githubJobsFromConfig) {
				...
				try {
					...
					List<CommitDetails> commitDetailList =
							gitHubClient.fetchAllCommits(gitHubProcessorItem, firstTimeRun, tool, proBasicConfig);
					...
					updateAssigneeNameForCommits(
							assigneeFlag, gitHubProcessorItem, commitDetailList, unsavedCommits);
					commitsRepo.saveAll(unsavedCommits);
					log.info(
							"Commits Saved For project {}->{}",
					...
					List<MergeRequests> mergeRequestsList =
							gitHubClient.fetchMergeRequests(
									gitHubProcessorItem, firstTimeRun, tool, proBasicConfig);
					updateAssigneeForMerge(
							assigneeFlag, gitHubProcessorItem, mergeRequestsList, unsavedMergeRequests);
					mergReqRepo.saveAll(unsavedMergeRequests);
					mergReqCount += unsavedMergeRequests.size();
					log.info(
							"MRs Saved For project {}->{}",
							proBasicConfig.getProjectName(),
		...
		MDC.put(
				"TotalGitHubProcessorJobExecutorTime",
	...
	private void updateAssigneeNameForCommits(
			boolean assigneeFlag,
			GitHubProcessorItem gitHubProcessorItem,
			List<CommitDetails> commitDetailList,
			Set<CommitDetails> unsavedCommits) {
		List<String> revisionNumbers =
				commitDetailList.stream()
						.map(CommitDetails::getRevisionNumber)
						.collect(Collectors.toList());
		List<CommitDetails> byProcessorItemIdAndRevisionNumberIn =
				commitsRepo.findByProcessorItemIdAndRevisionNumberIn(
						gitHubProcessorItem.getId(), revisionNumbers);
		log.info("Found Records of Commits in db ->{}", byProcessorItemIdAndRevisionNumberIn.size());
		commitDetailList.stream()
				.forEach(
						commit -> {
							Optional<CommitDetails> commitDetailsData =
									byProcessorItemIdAndRevisionNumberIn.stream()
											.filter(
													existing ->
															existing
																	.getRevisionNumber()
																	.equalsIgnoreCase(commit.getRevisionNumber()))
											.findFirst();
	...
	private boolean checkAssigneeFlag(
	...
	private void updateAssigneeForMerge(
			...
			GitHubProcessorItem gitHubProcessorItem,
			List<MergeRequests> mergeRequestsList,
			Set<MergeRequests> unsavedMerges) {
		Set<String> revisionNumbers =
				mergeRequestsList.stream()
						.map(MergeRequests::getRevisionNumber)
						.collect(Collectors.toSet());
		List<MergeRequests> byProcessorItemIdAndRevisionNumberIn =
				mergReqRepo.findByProcessorItemIdAndRevisionNumberIn(
						gitHubProcessorItem.getId(), revisionNumbers);
		log.info("Found Records of Merge in db ->{}", byProcessorItemIdAndRevisionNumberIn.size());
		mergeRequestsList.stream()
				.forEach(
						mergeRequests -> {
							Optional<MergeRequests> mergeRequestData =
									byProcessorItemIdAndRevisionNumberIn.stream()
											.filter(
													existing ->
															existing
																	.getRevisionNumber()
																	.equalsIgnoreCase(mergeRequests.getRevisionNumber()))
											.findFirst();
	...
	private GitHubProcessorItem getGitHubProcessorItem(
			ProcessorToolConnection tool, ObjectId processorId) {
		List<GitHubProcessorItem> gitHubProcessorItemList =
				gitHubProcessorItemRepository.findByProcessorIdAndToolConfigId(processorId, tool.getId());
		GitHubProcessorItem gitHubProcessorItem;
		if (CollectionUtils.isNotEmpty(gitHubProcessorItemList)) {
			gitHubProcessorItem = gitHubProcessorItemList.get(0);
		} else {
			gitHubProcessorItem =
					gitHubProcessorItemRepository.save(createProcessorItem(tool, processorId));
		}
	...
	private ProcessorExecutionTraceLog createTraceLog(String basicProjectConfigId) {
		...
		Optional<ProcessorExecutionTraceLog> existingTraceLogOptional =
				processorExecutionTraceLogRepository.findByProcessorNameAndBasicProjectConfigId(
						ProcessorConstants.GITHUB, basicProjectConfigId);
	...
	private void cacheRestClient(String cacheEndPoint, String cacheName) {
		...
		UriComponentsBuilder uriBuilder =
				UriComponentsBuilder.fromHttpUrl(gitHubConfig.getCustomApiBaseUrl());
		uriBuilder.path("/");
