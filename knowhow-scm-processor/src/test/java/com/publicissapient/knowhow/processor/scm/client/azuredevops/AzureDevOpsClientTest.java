...
class AzureDevOpsClientTest {
	...
	void testGetRepository_Success() throws Exception {
		...
		try (MockedConstruction<AzDClientApi> mockedClient =
				mockConstruction(
						AzDClientApi.class, (mock, context) -> when(mock.getGitApi()).thenReturn(mockGitApi))) {
	...
	void testGetRepository_Failure_ThrowsException() throws AzDException {
		GitDetails mockGitApi = mock(GitDetails.class);
		when(mockGitApi.getRepository(TEST_REPO))
	...
	void testFetchCommits_Success_WithDateFiltering() throws Exception {
		...
		try (MockedConstruction<AzDClientApi> mockedClient =
				mockConstruction(
						AzDClientApi.class, (mock, context) -> when(mock.getGitApi()).thenReturn(mockGitApi))) {
			List<GitCommitRef> result =
					azureDevOpsClient.fetchCommits(
							TEST_ORG, TEST_PROJECT, TEST_REPO, TEST_BRANCH, TEST_TOKEN, since, until);
			assertNotNull(result);
	...
	void testFetchCommits_WithNullDates() throws Exception {
		...
		try (MockedConstruction<AzDClientApi> mockedClient =
			...
			List<GitCommitRef> result =
					azureDevOpsClient.fetchCommits(
							TEST_ORG, TEST_PROJECT, TEST_REPO, TEST_BRANCH, TEST_TOKEN, since, null);
			assertNotNull(result);
					...
					assertThrows(
							...
							() ->
									azureDevOpsClient.fetchCommits(
											TEST_ORG, TEST_PROJECT, TEST_REPO, TEST_BRANCH, TEST_TOKEN, since, null));
			assertTrue(
					exception.getMessage().contains("Azure DevOps authentication failed")
	...
	void testFetchPullRequests_Success() throws Exception {
		...
		try (MockedConstruction<AzDClientApi> mockedClient =
				mockConstruction(
						AzDClientApi.class, (mock, context) -> when(mock.getGitApi()).thenReturn(mockGitApi))) {
			List<GitPullRequest> result =
					azureDevOpsClient.fetchPullRequests(
							TEST_ORG, TEST_PROJECT, TEST_REPO, TEST_TOKEN, since, TEST_BRANCH);
			assertNotNull(result);
			assertEquals(2, result.size());
		}
	}
	...
	void testFetchPullRequests_WithNullSince() throws Exception {
		...
		try (MockedConstruction<AzDClientApi> mockedClient =
			...
			List<GitPullRequest> result =
					azureDevOpsClient.fetchPullRequests(
							TEST_ORG, TEST_PROJECT, TEST_REPO, TEST_TOKEN, null, TEST_BRANCH);
			assertNotNull(result);
			assertEquals(1, result.size());
		}
	}
		...
		try (MockedConstruction<AzDClientApi> mockedClient =
			...
			assertTrue(
					exception.getMessage().contains("Azure DevOps authentication failed")
							|| exception
									.getMessage()
									.contains("Failed to fetch pull requests from Azure DevOps"));
		}
	}
	...
	void testGetPullRequestPickupTime_Success() throws Exception {
		...
		String threadsJson =
				"{\"value\":[{\"comments\":[{\"publishedDate\":\"2024-06-15T12:00:00Z\"}]}]}";
		...
		long result =
				azureDevOpsClient.getPullRequestPickupTime(
						TEST_ORG, TEST_PROJECT, TEST_REPO, TEST_TOKEN, pr);
		assertTrue(result > 0);
	}
	...
	void testGetCommitDiffStats_Success() throws Exception {
		...
		try (MockedConstruction<AzDClientApi> mockedClient =
				mockConstruction(
						AzDClientApi.class, (mock, context) -> when(mock.getGitApi()).thenReturn(mockGitApi))) {
			ScmCommits.FileChange result =
					azureDevOpsClient.getCommitDiffStats(
							TEST_ORG, TEST_PROJECT, TEST_REPO, "commit123", TEST_TOKEN);
			assertNotNull(result);
		}
	}
