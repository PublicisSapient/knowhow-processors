...
public class JobOrchestrator {
	private static final String JOB_IS_NOT_REGISTERED_EXCEPTION_MESSAGE =
			"Job '%s' is not registered";
	...
	public JobExecutionResponseRecord runJob(String jobName) {
		...
		try {
			...
			executionTraceLog.setErrorDetailList(
					List.of(ErrorDetail.builder().error(errorMessage).build()));
			this.jobExecutionTraceLogService.updateJobExecution(executionTraceLog);
			log.error(errorMessage);
			throw new InternalServerErrorException(
					String.format(
							"Encountered unexpected error while trying to run job with name '%s'", jobName));
		}
	}

	public boolean jobIsCurrentlyRunning(String jobName) {
		return this.jobExecutionTraceLogService.isJobCurrentlyRunning(
				ProcessorConstants.AI_DATA, jobName);
	}
	...
	private void validateJobCanBeRun(String jobName) {
		...
		if (!jobIsEnabled(jobName)) {
			throw new JobNotEnabledException(
					String.format("Job '%s' did not run because is disabled", jobName));
		}
		if (jobIsCurrentlyRunning(jobName)) {
			throw new ConcurrentJobExecutionException(
					String.format("Job '%s' is already running", jobName));
		}
	}
