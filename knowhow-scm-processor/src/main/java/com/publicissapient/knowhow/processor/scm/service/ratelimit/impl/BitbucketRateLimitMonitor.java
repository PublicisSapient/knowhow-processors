...
/**
 * Rate limit monitor for Bitbucket API. Supports both Bitbucket Cloud and Bitbucket Server
 * (on-premise).
 */
@Component
		...
		try {
			...
			if (apiUrl.contains("api.bitbucket.org")) {
				...
				return new RateLimitStatus(
						PLATFORM_NAME, 4000, 5000, System.currentTimeMillis() + 3600000, 1000);
			} else {
				// Bitbucket Server - even more conservative
				return new RateLimitStatus(
						PLATFORM_NAME, 800, 1000, System.currentTimeMillis() + 3600000, 200);
			}

		} catch (WebClientResponseException e) {
			logger.warn(
					"Rate limit check failed for Bitbucket: {} - {}", e.getStatusCode(), e.getMessage());

			if (e.getStatusCode() == HttpStatus.UNAUTHORIZED) {
				logger.error("Unauthorized access to Bitbucket API. Please check your credentials.");
				throw new RateLimitExceededException(
						PLATFORM_NAME, 0, 1000, 100.0, System.currentTimeMillis() + 3600000);
			} else if (e.getStatusCode() == HttpStatus.FORBIDDEN) {
				logger.error("Forbidden access to Bitbucket API. Rate limit may be exceeded.");
				throw new RateLimitExceededException(
						PLATFORM_NAME, 1000, 1000, 100.0, System.currentTimeMillis() + 3600000);
			...
			return new RateLimitStatus(
					PLATFORM_NAME, 500, 1000, System.currentTimeMillis() + 3600000, 500);
