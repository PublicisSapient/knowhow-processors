 ...
 *
 * <p>This service acts as a facade for all rate limit monitoring operations, delegating to
 * platform-specific monitors while providing a unified interface.
 *
 * <p>Follows the Strategy pattern by using different monitors for different platforms.
 */
@Service
public class RateLimitService {
	...
	public RateLimitService(List<RateLimitMonitor> monitorList) {
		this.monitors =
				monitorList.stream()
						.collect(
								Collectors.toMap(
										monitor -> monitor.getPlatformName().toLowerCase(), Function.identity()));

		logger.info(
				"Initialized RateLimitService with {} monitors: {}", monitors.size(), monitors.keySet());
		logger.info(
				"Rate limit configuration - enabled: {}, threshold: {}%, max-cooldown: {} hours, fail-on-excessive-cooldown: {}",
				rateLimitCheckEnabled, defaultThreshold * 100, maxCooldownHours, failOnExcessiveCooldown);
	}

	/**
	 * Check rate limit for a specific platform before making API calls. If rate limit threshold is
	 * exceeded, logs the information and waits for the platform's cooldown period.
	 *
	...
	public void checkRateLimit(String platform, String token, String repositoryName, String baseUrl) {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Rate limit status for {} ({}): {}/{} requests used ({}%), remaining: {}",
						platform,
						repositoryName,
						status.getUsed(),
						status.getLimit(),
			...
			if (status != null && status.exceedsThreshold(threshold)) {
				...
				logger.debug(
						"Rate limit check passed for platform: {} (repository: {})", platform, repositoryName);
			}
		} catch (Exception e) {
			logger.warn(
					"Failed to check rate limit for platform {} (repository: {}): {}",
					platform,
					repositoryName,
					e.getMessage());
	...
	/**
	 * Handles rate limit exceeded scenario by logging detailed information and waiting for the
	 * platform's cooldown period. The wait time is determined by the platform's rate limit reset
	 * time, not an arbitrary maximum.
	 *
	...
	private void handleRateLimitExceeded(
			String platform, RateLimitStatus status, double threshold, String repositoryName) {
		...
		LocalDateTime resetTime =
				LocalDateTime.ofInstant(Instant.ofEpochMilli(resetTimeMillis), ZoneId.systemDefault());
		...
		logger.warn(
				...
				status.getUsed(), status.getLimit(), status.getUsagePercentage() * 100);
		...
		if (waitTimeMillis > 0) {
			...
			if (waitTimeMillis > maxCooldownMillis) {
				logger.error(
						"Platform cooldown time ({} hours) exceeds maximum reasonable cooldown ({} hours)",
						waitTimeHours,
						maxCooldownHours);
				logger.error(
						"This may indicate an issue with the platform's rate limit reset time or system clock");

				if (failOnExcessiveCooldown) {
					logger.error(
							"Failing operation due to excessive cooldown time and fail-on-excessive-cooldown=true");
					throw new RateLimitExceededException(
							platform, status.getUsed(), status.getLimit(), threshold, status.getResetTime());
				} else {
					logger.warn(
							"Skipping rate limit wait due to excessive cooldown time. Continuing with reduced API calls.");
					logger.warn(
							"Consider checking system clock synchronization or platform status for: {}",
							platform);
					return;
				}
			}
			...
			logger.warn(
					"Waiting for platform's rate limit cooldown: {} seconds ({} minutes, {} hours)",
					waitTimeSeconds,
					waitTimeMinutes,
					waitTimeHours);
			logger.warn("Thread will sleep until platform rate limit resets...");
			logger.info(
					"RATE LIMIT SLEEP STARTED - Platform: {}, Repository: {}, Cooldown Time: {} hours {} minutes",
					platform,
					repositoryName,
					waitTimeHours,
					waitTimeMinutes % 60);
			...
			try {
				...
				logger.info(
						"RATE LIMIT SLEEP COMPLETED - Platform: {}, Repository: {}", platform, repositoryName);
				logger.info(
						"Platform rate limit cooldown completed for: {} (repository: {})",
						platform,
						repositoryName);

			} catch (InterruptedException e) {
				logger.error(
						"Thread interrupted while waiting for platform rate limit cooldown: {}",
						e.getMessage());
				logger.error(
						"RATE LIMIT SLEEP INTERRUPTED - Platform: {}, Repository: {}",
						platform,
						repositoryName);
