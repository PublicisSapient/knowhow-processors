...
public class RallyIssueProcessorImpl implements RallyIssueProcessor {
	...
	private void processJiraIssueData(
			JiraIssue jiraIssue, HierarchicalRequirement hierarchicalRequirement) {
		...
		jiraIssue.setChangeDate(
				RallyProcessorUtil.getFormattedDate(hierarchicalRequirement.getLastUpdateDate()));
		...
		jiraIssue.setCreatedDate(
				RallyProcessorUtil.getFormattedDate(hierarchicalRequirement.getCreationDate()));
	}
	...
	private void setDefectIssueType(
		...
		if (CollectionUtils.isNotEmpty(fieldMapping.getJiradefecttype())
				&& fieldMapping.getJiradefecttype().stream()
						.anyMatch(hierarchicalRequirement.getType()::equalsIgnoreCase)) {
	...
	public JiraIssue convertToJiraIssue(
		...
		else jiraIssue.setTypeName(hierarchicalRequirement.getType());
		...
		if (hierarchicalRequirement.getIteration() != null) {
			...
			jiraIssue.setSprintID(
					hierarchicalRequirement.getIteration().getObjectID()
	...
	private void setStoryLinkWithDefect(
			HierarchicalRequirement hierarchicalRequirement, JiraIssue jiraIssue) {
		...
		if (NormalizedJira.DEFECT_TYPE.getValue().equalsIgnoreCase(jiraIssue.getTypeName())
			...
			if (hierarchicalRequirement.getAdditionalProperties() != null) {
				...
				for (String key :
						Arrays.asList("Requirement", "Parent", "WorkProduct", "Requirement.FormattedID")) {
			...
			if (!defectStorySet.isEmpty()) {
				log.debug(
						"Found {} linked stories for defect {}: {}",
						defectStorySet.size(),
						hierarchicalRequirement.getFormattedID(),
						defectStorySet);
	...
	private static void setDefectStory(
			HierarchicalRequirement hierarchicalRequirement, Set<String> defectStorySet) {
		if (hierarchicalRequirement.getRequirement() != null) {
			...
			if (requirement.getFormattedID() != null) {
				...
				log.debug(
						"Found direct Requirement link for defect {}: {}",
						hierarchicalRequirement.getFormattedID(),
						requirement.getFormattedID());
			}
		}
	}
}
