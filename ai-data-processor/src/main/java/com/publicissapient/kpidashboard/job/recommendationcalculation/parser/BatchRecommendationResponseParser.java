...
/**
 * Parser for batch processor AI Gateway responses. Converts AI-generated JSON responses into
...
public class BatchRecommendationResponseParser {
	...
	/**
	 * Parses AI response into a Recommendation object. Validates response content and structure.
	 *
	 * @param response ChatGenerationResponseDTO from AI Gateway
	 * @return parsed and validated Recommendation object
	 * @throws IllegalArgumentException if response is null
	 * @throws IllegalStateException if response content is empty, JSON extraction fails, or required
	 *     fields missing
	 * @throws Exception if JSON parsing fails or other processing errors occur
	 */
	 ...
	 * @throws IllegalStateException if content is empty, JSON extraction fails, or required fields
	 *     missing
	...
	private Recommendation parseRecommendationContent(String aiResponse) throws Exception {
		...
		if (recommendationsArray != null
				&& recommendationsArray.isArray()
				&& !recommendationsArray.isEmpty()) {
			return parseAndValidateRecommendation(recommendationsArray.get(0));
		}

		// Missing required fields
		throw new IllegalStateException(
				"AI response missing required fields: title and description must be non-empty");
	}
	...
	/**
	 * Extracts JSON content from AI response by removing markdown code blocks. Handles responses
	 * wrapped in ```json``` markdown blocks with fallback logic.
	 *
	...
	private String extractJsonContent(String aiResponse) {
		...
		if (content.startsWith(MARKDOWN_CODE_FENCE)) {
			...
			if (extracted == null) {
				log.warn(
						"{} Malformed markdown fence - attempting fallback extraction",
	...
	/**
	 * Parses a JSON node into a Recommendation object. Extracts all fields directly from AI response.
	 *
	 * @param node the JSON node containing recommendation data
	...
	private Recommendation parseRecommendationNode(JsonNode node) {
		...
		Severity severity =
				Optional.ofNullable(getTextValue(node, SEVERITY))
						.map(String::toUpperCase)
						.flatMap(this::parseSeverity)
						.orElse(null);

		// Parse action plans
		List<ActionPlan> actionPlans =
				Optional.ofNullable(node.get(ACTION_PLANS))
						.filter(JsonNode::isArray)
						.map(this::parseActionPlans)
						.orElse(null);
		...
		return Recommendation.builder()
				...
				.severity(severity)
				.timeToValue(getTextValue(node, TIME_TO_VALUE))
	...
	private Optional<Severity> parseSeverity(String severityStr) {
		try {
			...
			log.debug(
					"{} Invalid severity value '{}' in AI response - will be validated",
					JobConstants.LOG_PREFIX_RECOMMENDATION,
					severityStr);
	...
	/**
	 * Parses action plans from JSON array node. Filters out action plans with empty or blank
	 * title/description to ensure data quality.
	 *
	 * @param actionPlansNode the JSON array node containing action plans
	...
	private List<ActionPlan> parseActionPlans(JsonNode actionPlansNode) {
		...
		if (skippedCount > 0) {
			log.debug(
					"{} Filtered out {} action plan(s) with empty content from {} total",
					JobConstants.LOG_PREFIX_RECOMMENDATION,
					skippedCount,
					actionPlansNode.size());
		}
	...
	/**
	 * Safely extracts text value from JSON node. Returns null if field doesn't exist or is null.
	 *
