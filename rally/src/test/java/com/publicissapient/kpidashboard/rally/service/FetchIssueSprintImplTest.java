...
public class FetchIssueSprintImplTest {
	...
	public void testFetchIssuesSprintBasedOnJql() throws InterruptedException {
		...
		ResponseEntity<RallyResponse> mockResponseEntity =
		...
		// Execute
		List<HierarchicalRequirement> result =
				fetchIssueSprint.fetchIssuesSprintBasedOnJql(projectConfig, 1, sprintId);
	...
	public void testFetchIterationDetailsSuccess() throws Exception {
		...
		java.lang.reflect.Method method =
				FetchIssueSprintImpl.class.getDeclaredMethod(
	...
	public void testGetHierarchicalRequirements() throws Exception {
		...
		ResponseEntity<RallyResponse> hrResponseEntity =
				new ResponseEntity<>(hrResponse, HttpStatus.OK);
		...
		ResponseEntity<RallyResponse> defectResponseEntity =
				new ResponseEntity<>(defectResponse, HttpStatus.OK);
		...
		ResponseEntity<RallyResponse> taskResponseEntity =
				new ResponseEntity<>(taskResponse, HttpStatus.OK);

		// Mock restTemplate.exchange calls
		when(restTemplate.exchange(
						contains("hierarchicalrequirement"),
						eq(HttpMethod.GET),
						any(),
						eq(RallyResponse.class)))
				.thenReturn(hrResponseEntity)
				.thenReturn(
						new ResponseEntity<>(
								new RallyResponse(), HttpStatus.OK)); // Empty response for second page

		when(restTemplate.exchange(
						contains("defect"), eq(HttpMethod.GET), any(), eq(RallyResponse.class)))
		...
		when(restTemplate.exchange(
						contains("task"), eq(HttpMethod.GET), any(), eq(RallyResponse.class)))
		...
		// Use reflection to access private method
		java.lang.reflect.Method method =
				FetchIssueSprintImpl.class.getDeclaredMethod("getHierarchicalRequirements", int.class);
		method.setAccessible(true);

		// Execute
		@SuppressWarnings("unchecked")
		List<HierarchicalRequirement> result =
				(List<HierarchicalRequirement>) method.invoke(fetchIssueSprint, pageStart);
