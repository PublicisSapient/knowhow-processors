...
/** GitLab implementation of RateLimitMonitor. Monitors and validates GitLab API rate limits. */
@Component
@Slf4j
	...
	public RateLimitStatus checkRateLimit(String token, String baseUrl)
		...
		try (GitLabApi gitLabApi = new GitLabApi(baseUrl, token)) {
			...
			if (e.getHttpStatus() == 401) {
				...
				log.warn(
						"GitLab API access forbidden - possible rate limit or permission issue: {}",
						e.getMessage());
				...
				log.warn(
						"GitLab API endpoint not found (404) - this might indicate token permissions issue or wrong API URL. Using default rate limit status.");
				...
				log.error(
						"Error checking GitLab rate limit: {} (HTTP {})", e.getMessage(), e.getHttpStatus());
