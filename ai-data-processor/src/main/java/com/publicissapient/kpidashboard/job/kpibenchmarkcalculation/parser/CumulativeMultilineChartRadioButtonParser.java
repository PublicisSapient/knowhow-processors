                        ...
                        .forEach(dataCountGroup -> {
                            if (dataCountGroup != null && dataCountGroup.getDataGroup() != null) {
                                ...
                                if (CollectionUtils.isNotEmpty(dataCountGroupList)) {
                                    List<DataCount> dataCountList = dataCountGroupList.stream()
                                            .flatMap(dataGroup -> dataGroup.getValue().stream())
                                            .toList();
                                    if (!dataCountList.isEmpty()) {
                                        Map<String, List<Double>> dataPoints =
                                                extractDataPoints(dataCountList);
                                        dataPoints.forEach(
                                                (key, value) ->
                                                        valueList.put(key + "#" + dataCountGroup.getFilter1(),
                                                                value));
                                    }
