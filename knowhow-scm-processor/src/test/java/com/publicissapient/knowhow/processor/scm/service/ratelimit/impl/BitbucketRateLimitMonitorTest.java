...
class BitbucketRateLimitMonitorTest {
	...
	void setUp() {
		...
		ReflectionTestUtils.setField(
				bitbucketRateLimitMonitor, "bitbucketApiUrl", "https://api.bitbucket.org/2.0");
	...
	void testCheckRateLimit_BitbucketCloud_ReturnsCloudLimits() {
		RateLimitStatus status =
				bitbucketRateLimitMonitor.checkRateLimit(TEST_TOKEN, BITBUCKET_CLOUD_URL);
	...
	void testCheckRateLimit_BitbucketServer_ReturnsServerLimits() {
		RateLimitStatus status =
				bitbucketRateLimitMonitor.checkRateLimit(TEST_TOKEN, BITBUCKET_SERVER_URL);
	...
	void testCheckRateLimit_Unauthorized_ThrowsRateLimitExceededException() {
		BitbucketRateLimitMonitor testMonitor =
				new BitbucketRateLimitMonitor() {
					@Override
						...
						try {
							...
							if (e.getStatusCode() == HttpStatus.UNAUTHORIZED) {
								throw new RateLimitExceededException(
										"Bitbucket", 0, 1000, 100.0, System.currentTimeMillis() + 3600000);
							}
							return new RateLimitStatus(
									"Bitbucket", 500, 1000, System.currentTimeMillis() + 3600000, 500);
						}
					}
				};
					...
					public RateLimitStatus checkRateLimit(String token, String baseUrl) {
						try {
							...
							if (e.getStatusCode() == HttpStatus.FORBIDDEN) {
								throw new RateLimitExceededException(
										"Bitbucket", 1000, 1000, 100.0, System.currentTimeMillis() + 3600000);
							}
							return new RateLimitStatus(
									"Bitbucket", 500, 1000, System.currentTimeMillis() + 3600000, 500);
						}
	...
	void testCheckRateLimit_BitbucketCloudWithApiInUrl() {
		RateLimitStatus status =
				bitbucketRateLimitMonitor.checkRateLimit(
						TEST_TOKEN, "https://api.bitbucket.org/2.0/repositories");
	...
	void testCheckRateLimit_BitbucketServerCustomUrl() {
		RateLimitStatus status =
				bitbucketRateLimitMonitor.checkRateLimit(TEST_TOKEN, "https://my-bitbucket.company.com");
