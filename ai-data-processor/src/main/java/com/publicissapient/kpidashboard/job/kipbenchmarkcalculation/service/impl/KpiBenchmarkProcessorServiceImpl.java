...
public class KpiBenchmarkProcessorServiceImpl implements KpiBenchmarkProcessorService {
	...
	public KpiBenchmarkProcessorServiceImpl(KpiParserStrategy kpiParserStrategy, KnowHOWClient knowHOWClient,
	...
	public List<KpiBenchmarkValues> getKpiWiseBenchmarkValues(List<KpiDataDTO> kpiDataDTOList) {
		...
		return kpiElementList.stream().collect(Collectors.groupingBy(KpiElement::getKpiId)).entrySet().stream()
				.map(entry -> createKpiBenchmarkValues(entry.getKey(), entry.getValue(), kpiFilterMap)).toList();
	}
	...
	private List<KpiElement> fetchKpiElements(List<KpiDataDTO> kpiDataDTOList) {
		return projectBasicConfigRepository.findAll().stream()
				...
				.flatMap(request -> knowHOWClient.getKpiIntegrationValues(request).stream()).toList();
	}

	private Map<String, String> createKpiFilterMap(List<KpiDataDTO> kpiDataDTOList) {
		return kpiDataDTOList.stream().collect(
				Collectors.toMap(KpiDataDTO::kpiId, dto -> dto.kpiFilter() != null ? dto.kpiFilter() : "default"));
	}

	private KpiBenchmarkValues createKpiBenchmarkValues(String kpiId, List<KpiElement> kpiElements,
			Map<String, String> kpiFilterMap) {
		try {
			log.debug("Calculating Benchmark for KPI ID: {}", kpiId);
			Map<String, List<Double>> allDataPoints = kpiElements.stream()
					.map(element -> processKpiData(element, kpiFilterMap.get(kpiId)))
					.flatMap(map -> map.entrySet().stream()).collect(Collectors.groupingBy(Map.Entry::getKey,
							Collectors.flatMapping(entry -> entry.getValue().stream(), Collectors.toList())));

			List<BenchmarkPercentiles> filterWiseBenchmark = allDataPoints.entrySet().stream()
					.map(entry -> createBenchmarkPercentiles(entry.getValue(), entry.getKey())).toList();

			return KpiBenchmarkValues.builder().kpiId(kpiId).filterWiseBenchmarkValues(filterWiseBenchmark)
	...
	private Map<String, List<Double>> processKpiData(KpiElement kpiElement, String kpiFilter) {
		return Optional.ofNullable(kpiElement.getTrendValueList()).filter(List.class::isInstance).map(
				trendValueList -> kpiParserStrategy.getParser(kpiFilter).getKpiDataPoints((List<?>) trendValueList))
				.orElse(Collections.emptyMap());
	}

	private BenchmarkPercentiles createBenchmarkPercentiles(List<Double> values, String filter) {
		return BenchmarkPercentiles.builder().filter(filter)
				.seventyPercentile(BenchmarkCalculation.percentile(values, 70))
				.eightyPercentile(BenchmarkCalculation.percentile(values, 80))
				.nintyPercentile(BenchmarkCalculation.percentile(values, 90)).build();
	}
	...
	private KpiRequest constructKpiRequest(List<KpiDataDTO> kpiDataDTOList, String projectNodeId) {
		...
		return KpiRequest.builder().kpiIdList(kpiDataDTOList.stream().map(KpiDataDTO::kpiId).toList())
				.label(CommonConstant.HIERARCHY_LEVEL_ID_PROJECT).ids(new String[] { projectNodeId })
				.selectedMap(selectedMap).level(5).build();
	}
