...
public class ScrumHandleAzureIssueHistory {
	...
	private List<JiraHistoryChangeLog> getJiraFieldChangeLogFromAdditionProps(
			List<Value> updateValueList, String jiraField) {
					...
					changelogItem
							...
							.forEach(
									(key, value) -> {
										...
										if (key.trim().equalsIgnoreCase(jiraField.trim())
												&& !changeLogValue.isEmpty()) {
	...
	public JiraHistoryChangeLog handleChangelogs(
			String key, Map<String, Object> value, Fields changelogItem) {
		...
		jiraHistoryChangeLog.setUpdatedOn(
				LocalDateTime.parse(
						AzureProcessorUtil.getFormattedDate(
								AzureProcessorUtil.deodeUTF8String(
		...
		if (CollectionUtils.isNotEmpty(updateValueList)) {
			for (Value history : updateValueList) {
				...
				if (changelogItem != null && changelogItem.getSystemState() != null) {
					JiraHistoryChangeLog jiraHistoryChangeLog = new JiraHistoryChangeLog();
					String oldValue =
							changelogItem.getSystemState().getOldValue() != null
		...
		if (CollectionUtils.isNotEmpty(updateValueList)) {
			for (Value history : updateValueList) {
				...
				if (changelogItem != null && changelogItem.getSystemIterationPath() != null) {
					JiraHistoryChangeLog jiraHistoryChangeLog = new JiraHistoryChangeLog();
					String oldValue =
							changelogItem.getSystemIterationPath().getOldValue() != null
									? getModifiedSprintsPath(changelogItem.getSystemIterationPath().getOldValue())
									: "";
					String newValue =
							changelogItem.getSystemIterationPath().getNewValue() != null
	...
	private String parseStringToLocalDateTime(String date) {
		return StringUtils.isEmpty(date)
				? ""
				: LocalDateTime.parse(
								AzureProcessorUtil.getFormattedDate(AzureProcessorUtil.deodeUTF8String(date)))
						.toString();
	}

	private List<JiraHistoryChangeLog> getDueDateChangeLog(
			List<Value> changeLogList, FieldMapping fieldMapping, Map<String, Object> fields) {
	...
	private List<JiraHistoryChangeLog> createDueDateChangeLogs(
			List<Value> changeLogList, String field) {
		List<JiraHistoryChangeLog> fieldHistoryLog = new ArrayList<>();
										...
										if (key.trim().equalsIgnoreCase(field)) {
											...
											jiraHistoryChangeLog.setChangedFrom(
													parseStringToLocalDateTime(
															handleStr((Map<String, Object>) value, OLD_VALUE)));
											jiraHistoryChangeLog.setChangedTo(
													parseStringToLocalDateTime(
															handleStr((Map<String, Object>) value, NEW_VALUE)));
											jiraHistoryChangeLog.setUpdatedOn(
													LocalDateTime.parse(
															AzureProcessorUtil.getFormattedDate(
																	AzureProcessorUtil.deodeUTF8String(
	...
	private String handleStr(Map<String, Object> changeValues, String valueState) {
		if (changeValues.containsKey(valueState)) return changeValues.get(valueState).toString();
		return "";
	}
	...
	public String convertToSeconds(Map<String, Object> changeValues, String valueState) {
		...
		try {
			if (changeValues.containsKey(valueState)) {
				int multipliedValue =
						(int) Double.parseDouble(changeValues.get(valueState).toString()) * 3600;
	...
	public void setJiraIssueCustomHistoryUpdationLog(
			JiraIssueCustomHistory jiraIssueCustomHistory,
			List<Value> updateValueList,
		...
		List<JiraHistoryChangeLog> assigneeChangeLog =
				getJiraFieldChangeLogFromAdditionProps(updateValueList, AzureConstants.ASSIGNEE);
		List<JiraHistoryChangeLog> priorityChangeLog =
				getJiraFieldChangeLogFromAdditionProps(updateValueList, AzureConstants.PRIORITY);
		List<JiraHistoryChangeLog> labelsChangeLog =
				getJiraFieldChangeLogFromAdditionProps(updateValueList, AzureConstants.LABEL);
		List<JiraHistoryChangeLog> workLog =
				getJiraFieldChangeLogFromAdditionProps(updateValueList, AzureConstants.WORKLOG);

		List<JiraHistoryChangeLog> dueDateChangeLog =
				getDueDateChangeLog(updateValueList, fieldMapping, fieldsMap);
