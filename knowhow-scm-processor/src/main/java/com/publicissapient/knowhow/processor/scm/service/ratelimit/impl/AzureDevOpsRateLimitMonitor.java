 ...
 *
 * <p>Azure DevOps has different rate limiting compared to other platforms: - Uses TFS (Team
 * Foundation Server) rate limiting - Typically allows 300 requests per minute per user - Rate
 * limits are enforced per organization and per user
 *
 * <p>Note: The azd library doesn't provide direct access to rate limit headers, so this
 * implementation provides estimated rate limit information.
 */
@Component
@Slf4j
public class AzureDevOpsRateLimitMonitor implements RateLimitMonitor {
	...
	public RateLimitStatus checkRateLimit(String token, String baseUrl) {
		...
		try {
			...
			if (connection.getOrganization() != null && !connection.getOrganization().isEmpty()) {
				...
				log.debug(
						"Azure DevOps rate limit status (estimated): {}/{} requests used",
						status.getUsed(),
						status.getLimit());
	...
	/**
	 * Creates a default rate limit status for cases where we can't determine the actual status. This
	 * is conservative to avoid hitting rate limits.
	 */
