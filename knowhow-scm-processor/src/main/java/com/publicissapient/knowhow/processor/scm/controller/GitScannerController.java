 ...
 *
 * <p>Provides endpoints for: - Triggering scmRepository scans - Checking scan status - Retrieving
 * scan results
 */
@RestController
...
public class GitScannerController {
	...
	public GitScannerController(
			GitScannerService gitScannerService, ScmProcessorScanExecutor scmProcessorScanExecutor) {
	...
	@Operation(
			...
			description =
					"Triggers a synchronous scan of a Git scmRepository to collect commits, merge requests, and user data. "
	...
	public ResponseEntity<GitScannerApiResponse<ScanResult>> scanRepository(
		...
		try {
			...
			GitScannerApiResponse<ScanResult> response =
					GitScannerApiResponse.<ScanResult>builder()
							.success(true)
	...
	public ResponseEntity<GitScannerApiResponse<HealthStatus>> health() {
		...
		GitScannerApiResponse<HealthStatus> response =
				GitScannerApiResponse.<HealthStatus>builder()
						.success(true)
						.message(SUCCESS_STATUS)
						.data(healthStatus)
						.timestamp(LocalDateTime.now())
	...
	public static class ScanRepositoryRequest {
		...
		@Schema(description = "Username for the Git platform account", example = "john.doe")
		@NotBlank(message = "Username is required")
		private String username;

		@Schema(description = "The branch to scan (typically 'main' or 'master')", example = "main")
		...
		@Schema(description = "The type of Git platform", example = "GITHUB")
	...
	@Operation(
			...
			description =
					"Processes SCM connection metadata for the specified connection ID. "
	...
	public ResponseEntity<GitScannerApiResponse<ScanResult>> syncConnectionMetadata(
			@Parameter(
							description = "The connection ID (MongoDB ObjectId format)",
							required = true,
		...
		try {
			...
			GitScannerApiResponse<ScanResult> response =
					GitScannerApiResponse.<ScanResult>builder()
							.success(false)
							.message("Failed to process connection metadata: " + e.getMessage())
							.data(null)
							.timestamp(LocalDateTime.now())
							.build();
	...
	@Setter
	@Getter
	@Schema(name = "HealthStatus", description = "Health status information for the service")
	public static class HealthStatus {
		...
		@Schema(description = "Timestamp of the health check", example = "2024-01-15T10:30:00")
		private LocalDateTime timestamp;

		@Schema(description = "Additional health status message", example = "Git Scanner is running")
	...
	@Schema(
			...
			description =
					"Generic API response wrapper containing success status, message, data, and timestamp")
	public static class GitScannerApiResponse<T> {
		...
		@Schema(description = "Human-readable message describing the result", example = "Success")
		private String message;
	...
	@Schema(name = "ToolType", description = "Supported Git platform types")
