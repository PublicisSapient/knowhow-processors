...
class AzureDevOpsMergeRequestServiceImplTest {
	...
	@Mock private AzureDevOpsClient azureDevOpsClient;

	@Mock private AzureDevOpsCommonHelper commonHelper;

	@InjectMocks private AzureDevOpsMergeRequestServiceImpl service;
	...
	void fetchMergeRequests_Success() throws Exception {
		...
		verify(azureDevOpsClient)
				.fetchPullRequests("testOrg", "testProject", "testRepo", token, since, branchName);
	}

	@Test
	void fetchMergeRequests_WithNullBranch() throws Exception {
		when(azureDevOpsClient.fetchPullRequests(
						anyString(), anyString(), anyString(), anyString(), any(), isNull()))
		...
		assertNotNull(result);
		verify(azureDevOpsClient)
				.fetchPullRequests("testOrg", "testProject", "testRepo", token, since, null);
	}
	...
	void fetchMergeRequests_WithUrl() throws Exception {
		GitPullRequest azurePR = createMockGitPullRequest();
		when(azurePR.getUrl())
				.thenReturn("https://dev.azure.com/testOrg/testProject/_git/testRepo/pullrequest/123");
		...
		assertEquals(
				...
				result.get(0).getMergeRequestUrl());
	}
