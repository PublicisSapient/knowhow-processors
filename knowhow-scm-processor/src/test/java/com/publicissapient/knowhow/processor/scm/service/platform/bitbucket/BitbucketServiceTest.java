...
class BitbucketServiceTest {
    ...
    void testFetchCommits_Success() throws PlatformApiException {
        ...
        when(bitbucketClient.fetchCommits(anyString(), anyString(), anyString(), anyString(), 
                anyString(), any(), anyString())).thenReturn(mockCommits);
        when(bitbucketClient.fetchCommitDiffs(anyString(), anyString(), anyString(), 
                anyString(), anyString(), anyString())).thenReturn("mock diff content");
        ...
        // Act
        List<ScmCommits> result = bitbucketService.fetchCommits(toolConfigId, gitUrlInfo, 
                "main", token, since, until);
        ...
        verify(bitbucketClient).fetchCommits("testowner", "testrepo", "main", 
                "testuser", "testpassword", since, "https://bitbucket.org/testowner/testrepo.git");
    }

    @Test
    void testFetchCommits_WithRepositoryUrlContext() throws PlatformApiException {
        // Arrange
        String contextUrl = "https://custom.bitbucket.com/owner/repo.git";
        ...
        assertNotNull(result);
        verify(bitbucketClient).fetchCommits("testowner", "testrepo", "main", 
                "testuser", "testpassword", since, contextUrl);
    ...
    void testFetchCommits_PlatformApiException() throws PlatformApiException {
        ...
        when(bitbucketClient.fetchCommits(anyString(), anyString(), anyString(), anyString(), 
                anyString(), any(), anyString())).thenThrow(exception);
    ...
    void testFetchCommits_GenericException() throws PlatformApiException {
        // Arrange
        when(bitbucketClient.fetchCommits(anyString(), anyString(), anyString(), anyString(), 
                anyString(), any(), anyString())).thenThrow(new RuntimeException("Network error"));
    ...
    void testFetchCommits_WithStatsAndNoFileChanges() throws PlatformApiException {
        ...
        when(bitbucketClient.fetchCommits(anyString(), anyString(), anyString(), anyString(), 
                anyString(), any(), anyString())).thenReturn(mockCommits);
        when(bitbucketClient.fetchCommitDiffs(anyString(), anyString(), anyString(), 
                anyString(), anyString(), anyString())).thenThrow(new RuntimeException("Diff error"));
        when(bitbucketClient.getBitbucketParser(anyBoolean())).thenReturn(bitbucketParser);
    ...
    void testFetchCommits_WithoutStatsButWithFileChanges() throws PlatformApiException {
        ...
        assertNotNull(result);
        assertEquals(1, result.size());
        assertEquals(8, result.get(0).getAddedLines()); // Sum from file changes
        assertEquals(3, result.get(0).getRemovedLines()); // Sum from file changes
    }
    ...
    void testFetchMergeRequests_Success() throws PlatformApiException {
        ...
        when(bitbucketClient.fetchPullRequests(anyString(), anyString(), anyString(), anyString(), 
                anyString(), any(), anyString())).thenReturn(mockPRs);
        when(bitbucketClient.fetchPullRequestDiffs(anyString(), anyString(), any(), 
                anyString(), anyString(), anyString())).thenReturn("mock diff content");
        when(bitbucketClient.getBitbucketParser(anyBoolean())).thenReturn(bitbucketParser);
        when(bitbucketParser.parsePRDiffToFileChanges(anyString())).thenReturn(createMockPRStats());

        // Act
        List<ScmMergeRequests> result = bitbucketService.fetchMergeRequests(toolConfigId, gitUrlInfo, 
                "main", token, since, until);
    ...
    void testFetchMergeRequests_Exception() throws PlatformApiException {
        // Arrange
        when(bitbucketClient.fetchPullRequests(anyString(), anyString(), anyString(), anyString(), 
                anyString(), any(), anyString())).thenThrow(new RuntimeException("API Error"));
    ...
    private List<ScmCommits.FileChange> createMockFileChanges() {
        ...
        ScmCommits.FileChange change1 = ScmCommits.FileChange.builder()
    ...
    private List<ScmCommits.FileChange> createMockFileChangesWithLines() {
        ...
        ScmCommits.FileChange change2 = ScmCommits.FileChange.builder()
                .filePath("file2.java")
                .addedLines(3)
                .removedLines(1)
                .build();
