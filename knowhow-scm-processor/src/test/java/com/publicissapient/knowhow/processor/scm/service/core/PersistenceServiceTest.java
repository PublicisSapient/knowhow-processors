...
class PersistenceServiceTest {
	...
	@Mock private ScmUserRepository userRepository;
	...
	void testSaveUser_NewUser_Success() throws DataProcessingException {
		// Arrange
		when(userRepository.findByProcessorItemIdAndUsername(any(), any()))
	...
	//        // Arrange
	//        User existingUser = User.builder()// CHANGE: Remove String.valueOf() - id should be
	// ObjectId not String
	//                .username("testuser")
	//                .email("test@example.com")
	//                .displayName("Test User")
	...
	void testSaveUser_DuplicateKeyException_ThrowsDataProcessingException() {
		...
		DataProcessingException exception =
				assertThrows(DataProcessingException.class, () -> persistenceService.saveUser(testUser));
	...
	void testFindOrCreateUser_ExistingUser_ReturnsUser() {
		...
		// Act
		User result =
				persistenceService.findOrCreateUser(
						"test-repo", "testuser", "test@example.com", "Test User", processorItemId);
	...
	void testSaveCommits_AllNewCommits_Success() throws DataProcessingException {
		...
		when(commitRepository.findByProcessorItemIdAndSha(processorItemId, "abc123"))
				.thenReturn(Optional.empty());
		when(commitRepository.save(any(ScmCommits.class)))
				.thenAnswer(
						invocation -> {
		...
		verify(commitRepository)
				.save(argThat(commit -> commit.getCreatedAt() != null && commit.getUpdatedAt() != null));
	}
	...
	void testSaveCommits_Exception_ThrowsDataProcessingException() {
		...
		// Act & Assert
		DataProcessingException exception =
				assertThrows(DataProcessingException.class, () -> persistenceService.saveCommits(commits));
	...
	void testSaveMergeRequests_AllNew_Success() throws DataProcessingException {
		...
		when(mergeRequestRepository.findByProcessorItemIdAndExternalId(processorItemId, "MR-123"))
				.thenReturn(Optional.empty());
		when(mergeRequestRepository.save(any(ScmMergeRequests.class)))
				.thenAnswer(
						invocation -> {
	...
	void testSaveMergeRequests_Mixed_UpdatesAndCreates() throws DataProcessingException {
		...
		verify(mergeRequestRepository)
				.save(argThat(mr -> "Test MR".equals(mr.getTitle()) && mr.getId() != null));
	}
	...
	void testFindMergeRequestsByToolConfigIdAndState_Success() {
		...
		when(mergeRequestRepository.findByProcessorItemIdAndState(
						processorItemId, ScmMergeRequests.MergeRequestState.OPEN, pageable))
				.thenReturn(expectedPage);

		// Act
		Page<ScmMergeRequests> result =
	...
	void testUpdateMergeRequestFields_AllFieldsNonNull_UpdatesAll() throws DataProcessingException {
		...
		when(mergeRequestRepository.findByProcessorItemIdAndExternalId(
						source.getProcessorItemId(), source.getExternalId()))
				.thenReturn(Optional.of(target));
		when(mergeRequestRepository.save(any(ScmMergeRequests.class)))
				.thenAnswer(
						invocation -> {
