...
class RallyProcessorJobTest {
	...
	@Mock private RallyIssueRqlWriterListener jiraIssueJqlWriterListener;

	@Mock private JobListenerScrum jobListenerScrum;

	@Mock private RallyIssueSprintJobListener rallyIssueSprintJobListener;
	...
	@Mock private BuilderFactory builderFactory;

	@Mock private RallyProcessorConfig rallyProcessorConfig;

	@InjectMocks private RallyProcessorJob rallyProcessorJob;
	...
	void setUp() {
		...
		lenient()
				.when(builderFactory.getStepBuilder(anyString(), any(JobRepository.class)))
		...
		lenient()
				.when(stepBuilder.chunk(anyInt(), any(PlatformTransactionManager.class)))
		...
		lenient()
				.when(stepBuilder.tasklet(any(), any(PlatformTransactionManager.class)))
		...
		doReturn(taskletStepBuilder)
				.when(taskletStepBuilder)
				.listener(any(StepExecutionListener.class));
		...
		lenient()
				.when(builderFactory.getJobBuilder(anyString(), any(JobRepository.class)))
		...
		lenient()
				.when(simpleJobBuilder.listener(any(JobExecutionListener.class)))
